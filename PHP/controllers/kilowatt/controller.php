...
{		
	$homeBanners = $this->utility->getHomeBanners("kilowatt_homebanners");
	print json_encode($homeBanners); exit;
}

function getHomeAbout()
{
	$homeAbout = $this->utility->getHomeAbout("kilowatt_about");
	print json_encode($homeAbout); exit;
}

function getBenefits()
{
	$ourValues = $this->utility->getKilowattBenefit();
...
{
	$projects = $this->utility->getKilowattProjects();
	print json_encode($projects); exit;
}

function getHowWeWork()
{
	$howWeWork = $this->utility->getKilowattHowWeWork();
	print json_encode($howWeWork); exit;
}
...
{
	$contactInfo = $this->utility->getContactInfo("kilowatt_contact_info");
	print json_encode($contactInfo); exit;
}

function getFooterProjects()
{
	$projects = $this->utility->getKilowattProjects(6);
	print json_encode($projects); exit;
}
...
{
	$managementTeams = $this->utility->fetchManagementTeam("kilowatt_management_team");
	print json_encode($managementTeams); exit;
}

function fetchServices()
{
	$services = $this->utility->getKilowattServices();
