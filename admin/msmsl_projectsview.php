...
$msmsl_projects_view = NULL; // Initialize page object first

class cmsmsl_projects_view extends cmsmsl_projects {
	...
	// Page object name
	var $PageObjName = 'msmsl_projects_view';
	...
	function __construct() {
		...
		if (@$_GET["msmsl_project_id"] <> "") {
			$this->RecKey["msmsl_project_id"] = $_GET["msmsl_project_id"];
			$KeyUrl .= "&amp;msmsl_project_id=" . urlencode($this->RecKey["msmsl_project_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["msmsl_project_id"] <> "") {
				...
				$this->RecKey["msmsl_project_id"] = $this->msmsl_project_id->QueryStringValue;
			} elseif (@$_POST["msmsl_project_id"] <> "") {
				$this->msmsl_project_id->setFormValue($_POST["msmsl_project_id"]);
				$this->RecKey["msmsl_project_id"] = $this->msmsl_project_id->FormValue;
			} else {
				$sReturnUrl = "msmsl_projectslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "msmsl_projectslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "msmsl_projectslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($msmsl_projects_view)) $msmsl_projects_view = new cmsmsl_projects_view();

// Page init
$msmsl_projects_view->Page_Init();

// Page main
$msmsl_projects_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$msmsl_projects_view->Page_Render();
?>
...
var CurrentForm = fmsmsl_projectsview = new ew_Form("fmsmsl_projectsview", "view");

// Form_CustomValidate event
fmsmsl_projectsview.Form_CustomValidate = 
...
fmsmsl_projectsview.ValidateRequired = true;
<?php } else { ?>
fmsmsl_projectsview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmsmsl_projectsview.Lists["x_msmsl_project_category_id"] = {"LinkField":"x_msmsl_project_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_msmsl_project_category_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmsmsl_projectsview.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmsmsl_projectsview.Lists["x_status"].Options = <?php echo json_encode($msmsl_projects->status->Options()) ?>;
...
<?php $msmsl_projects_view->ExportOptions->Render("body") ?>
<?php
	foreach ($msmsl_projects_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $msmsl_projects_view->ShowPageHeader(); ?>
<?php
$msmsl_projects_view->ShowMessage();
?>
<form name="fmsmsl_projectsview" id="fmsmsl_projectsview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($msmsl_projects_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $msmsl_projects_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="msmsl_projects">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($msmsl_projects->msmsl_project_id->Visible) { // msmsl_project_id ?>
	<tr id="r_msmsl_project_id">
		<td><span id="elh_msmsl_projects_msmsl_project_id"><?php echo $msmsl_projects->msmsl_project_id->FldCaption() ?></span></td>
...
<?php echo $msmsl_projects->msmsl_project_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($msmsl_projects->msmsl_project_pic->Visible) { // msmsl_project_pic ?>
	<tr id="r_msmsl_project_pic">
		<td><span id="elh_msmsl_projects_msmsl_project_pic"><?php echo $msmsl_projects->msmsl_project_pic->FldCaption() ?></span></td>
...
<span>
<?php echo ew_GetFileViewTag($msmsl_projects->msmsl_project_pic, $msmsl_projects->msmsl_project_pic->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($msmsl_projects->msmsl_project_title->Visible) { // msmsl_project_title ?>
	<tr id="r_msmsl_project_title">
		<td><span id="elh_msmsl_projects_msmsl_project_title"><?php echo $msmsl_projects->msmsl_project_title->FldCaption() ?></span></td>
...
<?php echo $msmsl_projects->msmsl_project_title->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($msmsl_projects->msmsl_project_category_id->Visible) { // msmsl_project_category_id ?>
	<tr id="r_msmsl_project_category_id">
		<td><span id="elh_msmsl_projects_msmsl_project_category_id"><?php echo $msmsl_projects->msmsl_project_category_id->FldCaption() ?></span></td>
...
<?php echo $msmsl_projects->msmsl_project_category_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($msmsl_projects->sort_order->Visible) { // sort_order ?>
	<tr id="r_sort_order">
		<td><span id="elh_msmsl_projects_sort_order"><?php echo $msmsl_projects->sort_order->FldCaption() ?></span></td>
...
<?php echo $msmsl_projects->sort_order->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($msmsl_projects->status->Visible) { // status ?>
	<tr id="r_status">
		<td><span id="elh_msmsl_projects_status"><?php echo $msmsl_projects->status->FldCaption() ?></span></td>
...
<?php echo $msmsl_projects->status->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fmsmsl_projectsview.Init();
</script>
<?php
$msmsl_projects_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$msmsl_projects_view->Page_Terminate();
?>
