...
$skyview_services_skyview_service_pic_blobview = NULL; // Initialize page object first

class cskyview_services_skyview_service_pic_blobview extends cskyview_services {
	...
	// Page object name
	var $PageObjName = 'skyview_services_skyview_service_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('skyview_services', 'x_skyview_service_pic');

		// Show thumbnail
		$bShowThumbnail = (@$_GET["showthumbnail"] == "1");
		if (@$_GET["thumbnailwidth"] == "" && @$_GET["thumbnailheight"] == "") {
			$iThumbnailWidth = 129; // Set default width
			$iThumbnailHeight = 90; // Set default height
		} else {
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('skyview_service_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('skyview_service_pic')));
				if (trim(strval($this->Recordset->fields('skyview_service_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('skyview_service_pic') . "\"");
				}
...
// Create page object
if (!isset($skyview_services_skyview_service_pic_blobview)) $skyview_services_skyview_service_pic_blobview = new cskyview_services_skyview_service_pic_blobview();

// Page init
$skyview_services_skyview_service_pic_blobview->Page_Init();

// Page main
$skyview_services_skyview_service_pic_blobview->Page_Main();
?>
<?php
$skyview_services_skyview_service_pic_blobview->Page_Terminate();
?>
