...
$kilowatt_management_team_add = NULL; // Initialize page object first

class ckilowatt_management_team_add extends ckilowatt_management_team {
	...
	// Page object name
	var $PageObjName = 'kilowatt_management_team_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["kilowatt_management_team_id"] != "") {
				...
				$this->setKey("kilowatt_management_team_id", $this->kilowatt_management_team_id->CurrentValue); // Set up key
			} else {
				$this->setKey("kilowatt_management_team_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "kilowatt_management_teamview.php")
	...
	function LoadDefaultValues() {
		$this->kilowatt_management_team_pic->Upload->DbValue = NULL;
		$this->kilowatt_management_team_pic->OldValue = $this->kilowatt_management_team_pic->Upload->DbValue;
		$this->kilowatt_management_team_pic->CurrentValue = NULL; // Clear file related field
		$this->kilowatt_management_team_name->CurrentValue = NULL;
		$this->kilowatt_management_team_name->OldValue = $this->kilowatt_management_team_name->CurrentValue;
		$this->kilowatt_management_team_designation->CurrentValue = NULL;
		$this->kilowatt_management_team_designation->OldValue = $this->kilowatt_management_team_designation->CurrentValue;
		$this->kilowatt_management_team_content->CurrentValue = NULL;
		$this->kilowatt_management_team_content->OldValue = $this->kilowatt_management_team_content->CurrentValue;
		$this->sort_order->CurrentValue = NULL;
		$this->sort_order->OldValue = $this->sort_order->CurrentValue;
		$this->status->CurrentValue = "active";
		$this->kilowatt_management_team_linkedin->CurrentValue = NULL;
		$this->kilowatt_management_team_linkedin->OldValue = $this->kilowatt_management_team_linkedin->CurrentValue;
		$this->kilowatt_management_team_facebook->CurrentValue = NULL;
		$this->kilowatt_management_team_facebook->OldValue = $this->kilowatt_management_team_facebook->CurrentValue;
		$this->kilowatt_management_team_twitter->CurrentValue = NULL;
		$this->kilowatt_management_team_twitter->OldValue = $this->kilowatt_management_team_twitter->CurrentValue;
		$this->kilowatt_management_team_email->CurrentValue = NULL;
		$this->kilowatt_management_team_email->OldValue = $this->kilowatt_management_team_email->CurrentValue;
		$this->kilowatt_management_team_phone->CurrentValue = NULL;
		$this->kilowatt_management_team_phone->OldValue = $this->kilowatt_management_team_phone->CurrentValue;
	}
	...
	function RenderRow() {
		...
		$this->kilowatt_management_team_phone->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->kilowatt_management_team_pic);
	...
	function AddRow($rsold = NULL) {
		...
		if (!$this->kilowatt_management_team_pic->Upload->KeepFile) {
			$this->kilowatt_management_team_pic->Upload->DbValue = ""; // No need to delete old file
		...
		// kilowatt_management_team_name
		$this->kilowatt_management_team_name->SetDbValueDef($rsnew, $this->kilowatt_management_team_name->CurrentValue, "", FALSE);

		// kilowatt_management_team_designation
		$this->kilowatt_management_team_designation->SetDbValueDef($rsnew, $this->kilowatt_management_team_designation->CurrentValue, "", FALSE);

		// kilowatt_management_team_content
		$this->kilowatt_management_team_content->SetDbValueDef($rsnew, $this->kilowatt_management_team_content->CurrentValue, "", FALSE);
		...
		// kilowatt_management_team_linkedin
		$this->kilowatt_management_team_linkedin->SetDbValueDef($rsnew, $this->kilowatt_management_team_linkedin->CurrentValue, NULL, FALSE);

		// kilowatt_management_team_facebook
		$this->kilowatt_management_team_facebook->SetDbValueDef($rsnew, $this->kilowatt_management_team_facebook->CurrentValue, NULL, FALSE);

		// kilowatt_management_team_twitter
		$this->kilowatt_management_team_twitter->SetDbValueDef($rsnew, $this->kilowatt_management_team_twitter->CurrentValue, NULL, FALSE);

		// kilowatt_management_team_email
		$this->kilowatt_management_team_email->SetDbValueDef($rsnew, $this->kilowatt_management_team_email->CurrentValue, NULL, FALSE);

		// kilowatt_management_team_phone
		$this->kilowatt_management_team_phone->SetDbValueDef($rsnew, $this->kilowatt_management_team_phone->CurrentValue, NULL, FALSE);
		if (!$this->kilowatt_management_team_pic->Upload->KeepFile) {
			$this->kilowatt_management_team_pic->UploadPath = '../src/assets/images/kilowatt/team';
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->kilowatt_management_team_id->setDbValue($conn->Insert_ID());
				$rsnew['kilowatt_management_team_id'] = $this->kilowatt_management_team_id->DbValue;
				if (!$this->kilowatt_management_team_pic->Upload->KeepFile) {
					if (!ew_Empty($this->kilowatt_management_team_pic->Upload->Value)) {
...
// Create page object
if (!isset($kilowatt_management_team_add)) $kilowatt_management_team_add = new ckilowatt_management_team_add();

// Page init
$kilowatt_management_team_add->Page_Init();

// Page main
$kilowatt_management_team_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kilowatt_management_team_add->Page_Render();
?>
...
var CurrentForm = fkilowatt_management_teamadd = new ew_Form("fkilowatt_management_teamadd", "add");

// Validate form
fkilowatt_management_teamadd.Validate = function() {
...
fkilowatt_management_teamadd.Form_CustomValidate = 
...
fkilowatt_management_teamadd.ValidateRequired = true;
<?php } else { ?>
fkilowatt_management_teamadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkilowatt_management_teamadd.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_management_teamadd.Lists["x_status"].Options = <?php echo json_encode($kilowatt_management_team->status->Options()) ?>;
...
<?php $kilowatt_management_team_add->ShowPageHeader(); ?>
<?php
$kilowatt_management_team_add->ShowMessage();
?>
<form name="fkilowatt_management_teamadd" id="fkilowatt_management_teamadd" class="<?php echo $kilowatt_management_team_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kilowatt_management_team_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kilowatt_management_team_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fkilowatt_management_teamadd", "x_kilowatt_management_team_content", 35, 4, <?php echo ($kilowatt_management_team->kilowatt_management_team_content->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kilowatt_management_team_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fkilowatt_management_teamadd.Init();
</script>
<?php
$kilowatt_management_team_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kilowatt_management_team_add->Page_Terminate();
?>
