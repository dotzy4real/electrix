...
$kilowatt_projects_view = NULL; // Initialize page object first

class ckilowatt_projects_view extends ckilowatt_projects {
	...
	// Page object name
	var $PageObjName = 'kilowatt_projects_view';
	...
	function __construct() {
		...
		if (@$_GET["kilowatt_project_id"] <> "") {
			$this->RecKey["kilowatt_project_id"] = $_GET["kilowatt_project_id"];
			$KeyUrl .= "&amp;kilowatt_project_id=" . urlencode($this->RecKey["kilowatt_project_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["kilowatt_project_id"] <> "") {
				...
				$this->RecKey["kilowatt_project_id"] = $this->kilowatt_project_id->QueryStringValue;
			} elseif (@$_POST["kilowatt_project_id"] <> "") {
				$this->kilowatt_project_id->setFormValue($_POST["kilowatt_project_id"]);
				$this->RecKey["kilowatt_project_id"] = $this->kilowatt_project_id->FormValue;
			} else {
				$sReturnUrl = "kilowatt_projectslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "kilowatt_projectslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "kilowatt_projectslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($kilowatt_projects_view)) $kilowatt_projects_view = new ckilowatt_projects_view();

// Page init
$kilowatt_projects_view->Page_Init();

// Page main
$kilowatt_projects_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kilowatt_projects_view->Page_Render();
?>
...
var CurrentForm = fkilowatt_projectsview = new ew_Form("fkilowatt_projectsview", "view");

// Form_CustomValidate event
fkilowatt_projectsview.Form_CustomValidate = 
...
fkilowatt_projectsview.ValidateRequired = true;
<?php } else { ?>
fkilowatt_projectsview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkilowatt_projectsview.Lists["x_kilowatt_service_category_id"] = {"LinkField":"x_kilowatt_service_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_kilowatt_service_category_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_projectsview.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_projectsview.Lists["x_status"].Options = <?php echo json_encode($kilowatt_projects->status->Options()) ?>;
...
<?php $kilowatt_projects_view->ExportOptions->Render("body") ?>
<?php
	foreach ($kilowatt_projects_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $kilowatt_projects_view->ShowPageHeader(); ?>
<?php
$kilowatt_projects_view->ShowMessage();
?>
<form name="fkilowatt_projectsview" id="fkilowatt_projectsview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kilowatt_projects_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kilowatt_projects_view->Token ?>">
<?php } ?>
...
<?php if ($kilowatt_projects->kilowatt_project_id->Visible) { // kilowatt_project_id ?>
	<tr id="r_kilowatt_project_id">
		<td><span id="elh_kilowatt_projects_kilowatt_project_id"><?php echo $kilowatt_projects->kilowatt_project_id->FldCaption() ?></span></td>
		<td data-name="kilowatt_project_id"<?php echo $kilowatt_projects->kilowatt_project_id->CellAttributes() ?>>
<span id="el_kilowatt_projects_kilowatt_project_id">
<span<?php echo $kilowatt_projects->kilowatt_project_id->ViewAttributes() ?>>
<?php echo $kilowatt_projects->kilowatt_project_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kilowatt_projects->kilowatt_project_pic->Visible) { // kilowatt_project_pic ?>
	<tr id="r_kilowatt_project_pic">
		<td><span id="elh_kilowatt_projects_kilowatt_project_pic"><?php echo $kilowatt_projects->kilowatt_project_pic->FldCaption() ?></span></td>
...
<span>
<?php echo ew_GetFileViewTag($kilowatt_projects->kilowatt_project_pic, $kilowatt_projects->kilowatt_project_pic->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($kilowatt_projects->kilowatt_project_title->Visible) { // kilowatt_project_title ?>
	<tr id="r_kilowatt_project_title">
		<td><span id="elh_kilowatt_projects_kilowatt_project_title"><?php echo $kilowatt_projects->kilowatt_project_title->FldCaption() ?></span></td>
...
<?php echo $kilowatt_projects->kilowatt_project_title->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kilowatt_projects->kilowatt_service_category_id->Visible) { // kilowatt_service_category_id ?>
	<tr id="r_kilowatt_service_category_id">
		<td><span id="elh_kilowatt_projects_kilowatt_service_category_id"><?php echo $kilowatt_projects->kilowatt_service_category_id->FldCaption() ?></span></td>
...
<?php echo $kilowatt_projects->kilowatt_service_category_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kilowatt_projects->sort_order->Visible) { // sort_order ?>
	<tr id="r_sort_order">
		<td><span id="elh_kilowatt_projects_sort_order"><?php echo $kilowatt_projects->sort_order->FldCaption() ?></span></td>
...
<?php echo $kilowatt_projects->sort_order->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kilowatt_projects->status->Visible) { // status ?>
	<tr id="r_status">
		<td><span id="elh_kilowatt_projects_status"><?php echo $kilowatt_projects->status->FldCaption() ?></span></td>
...
<?php echo $kilowatt_projects->status->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fkilowatt_projectsview.Init();
</script>
<?php
$kilowatt_projects_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kilowatt_projects_view->Page_Terminate();
?>
