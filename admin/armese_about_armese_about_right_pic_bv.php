...
$armese_about_armese_about_right_pic_blobview = NULL; // Initialize page object first

class carmese_about_armese_about_right_pic_blobview extends carmese_about {
	...
	// Page object name
	var $PageObjName = 'armese_about_armese_about_right_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('armese_about', 'x_armese_about_right_pic');
		...
		// SQL constructor in armese_about class, armese_aboutinfo.php
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('armese_about_right_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('armese_about_right_pic')));
				if (trim(strval($this->Recordset->fields('armese_about_right_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('armese_about_right_pic') . "\"");
				}
...
// Create page object
if (!isset($armese_about_armese_about_right_pic_blobview)) $armese_about_armese_about_right_pic_blobview = new carmese_about_armese_about_right_pic_blobview();

// Page init
$armese_about_armese_about_right_pic_blobview->Page_Init();

// Page main
$armese_about_armese_about_right_pic_blobview->Page_Main();
?>
<?php
$armese_about_armese_about_right_pic_blobview->Page_Terminate();
?>
