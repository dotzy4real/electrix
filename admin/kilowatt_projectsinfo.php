...
$kilowatt_projects = NULL;

//
// Table class for kilowatt_projects
//
class ckilowatt_projects extends cTable {
	var $kilowatt_project_id;
	var $kilowatt_project_pic;
	var $kilowatt_project_title;
	...
	function __construct() {
		...
		$this->TableVar = 'kilowatt_projects';
		$this->TableName = 'kilowatt_projects';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`kilowatt_projects`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kilowatt_project_id
		$this->kilowatt_project_id = new cField('kilowatt_projects', 'kilowatt_projects', 'x_kilowatt_project_id', 'kilowatt_project_id', '`kilowatt_project_id`', '`kilowatt_project_id`', 19, -1, FALSE, '`kilowatt_project_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->kilowatt_project_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['kilowatt_project_id'] = &$this->kilowatt_project_id;

		// kilowatt_project_pic
		$this->kilowatt_project_pic = new cField('kilowatt_projects', 'kilowatt_projects', 'x_kilowatt_project_pic', 'kilowatt_project_pic', '`kilowatt_project_pic`', '`kilowatt_project_pic`', 200, -1, TRUE, '`kilowatt_project_pic`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['kilowatt_project_pic'] = &$this->kilowatt_project_pic;

		// kilowatt_project_title
		$this->kilowatt_project_title = new cField('kilowatt_projects', 'kilowatt_projects', 'x_kilowatt_project_title', 'kilowatt_project_title', '`kilowatt_project_title`', '`kilowatt_project_title`', 200, -1, FALSE, '`kilowatt_project_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['kilowatt_project_title'] = &$this->kilowatt_project_title;

		// kilowatt_service_category_id
		$this->kilowatt_service_category_id = new cField('kilowatt_projects', 'kilowatt_projects', 'x_kilowatt_service_category_id', 'kilowatt_service_category_id', '`kilowatt_service_category_id`', '`kilowatt_service_category_id`', 3, -1, FALSE, '`kilowatt_service_category_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// sort_order
		$this->sort_order = new cField('kilowatt_projects', 'kilowatt_projects', 'x_sort_order', 'sort_order', '`sort_order`', '`sort_order`', 3, -1, FALSE, '`sort_order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->sort_order->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sort_order'] = &$this->sort_order;

		// status
		$this->status = new cField('kilowatt_projects', 'kilowatt_projects', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`kilowatt_projects`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kilowatt_project_id', $rs))
				ew_AddFilter($where, ew_QuotedName('kilowatt_project_id', $this->DBID) . '=' . ew_QuotedValue($rs['kilowatt_project_id'], $this->kilowatt_project_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`kilowatt_project_id` = @kilowatt_project_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->kilowatt_project_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@kilowatt_project_id@", ew_AdjustSql($this->kilowatt_project_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "kilowatt_projectslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("kilowatt_projectsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("kilowatt_projectsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "kilowatt_projectsadd.php?" . $this->UrlParm($parm);
		else
			$url = "kilowatt_projectsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("kilowatt_projectsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("kilowatt_projectsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("kilowatt_projectsdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kilowatt_project_id:" . ew_VarToJson($this->kilowatt_project_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kilowatt_project_id->CurrentValue)) {
			$sUrl .= "kilowatt_project_id=" . urlencode($this->kilowatt_project_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = $isPost ? ew_StripSlashes(@$_POST["kilowatt_project_id"]) : ew_StripSlashes(@$_GET["kilowatt_project_id"]); // kilowatt_project_id
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->kilowatt_project_id->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->kilowatt_project_id->EditValue = $this->kilowatt_project_id->CurrentValue;
		$this->kilowatt_project_id->ViewCustomAttributes = "";
		...
		$this->kilowatt_project_title->EditValue = $this->kilowatt_project_title->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->kilowatt_project_id->Exportable) $Doc->ExportCaption($this->kilowatt_project_id);
					if ($this->kilowatt_project_pic->Exportable) $Doc->ExportCaption($this->kilowatt_project_pic);
					if ($this->kilowatt_project_title->Exportable) $Doc->ExportCaption($this->kilowatt_project_title);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->kilowatt_project_id->Exportable) $Doc->ExportField($this->kilowatt_project_id);
						if ($this->kilowatt_project_pic->Exportable) $Doc->ExportField($this->kilowatt_project_pic);
						if ($this->kilowatt_project_title->Exportable) $Doc->ExportField($this->kilowatt_project_title);
