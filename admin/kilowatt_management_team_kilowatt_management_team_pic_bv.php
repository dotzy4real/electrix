...
$kilowatt_management_team_kilowatt_management_team_pic_blobview = NULL; // Initialize page object first

class ckilowatt_management_team_kilowatt_management_team_pic_blobview extends ckilowatt_management_team {
	...
	// Page object name
	var $PageObjName = 'kilowatt_management_team_kilowatt_management_team_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('kilowatt_management_team', 'x_kilowatt_management_team_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('kilowatt_management_team_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('kilowatt_management_team_pic')));
				if (trim(strval($this->Recordset->fields('kilowatt_management_team_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('kilowatt_management_team_pic') . "\"");
				}
...
// Create page object
if (!isset($kilowatt_management_team_kilowatt_management_team_pic_blobview)) $kilowatt_management_team_kilowatt_management_team_pic_blobview = new ckilowatt_management_team_kilowatt_management_team_pic_blobview();

// Page init
$kilowatt_management_team_kilowatt_management_team_pic_blobview->Page_Init();

// Page main
$kilowatt_management_team_kilowatt_management_team_pic_blobview->Page_Main();
?>
<?php
$kilowatt_management_team_kilowatt_management_team_pic_blobview->Page_Terminate();
?>
