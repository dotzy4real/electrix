...
$board_directors_board_director_pic_blobview = NULL; // Initialize page object first

class cboard_directors_board_director_pic_blobview extends cboard_directors {
	...
	// Page object name
	var $PageObjName = 'board_directors_board_director_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('board_directors', 'x_board_director_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('board_director_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('board_director_pic')));
				if (trim(strval($this->Recordset->fields('board_director_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('board_director_pic') . "\"");
				}
...
// Create page object
if (!isset($board_directors_board_director_pic_blobview)) $board_directors_board_director_pic_blobview = new cboard_directors_board_director_pic_blobview();

// Page init
$board_directors_board_director_pic_blobview->Page_Init();

// Page main
$board_directors_board_director_pic_blobview->Page_Main();
?>
<?php
$board_directors_board_director_pic_blobview->Page_Terminate();
?>
