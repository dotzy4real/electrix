...
$msmsl_homebanners_edit = NULL; // Initialize page object first

class cmsmsl_homebanners_edit extends cmsmsl_homebanners {
	...
	// Page object name
	var $PageObjName = 'msmsl_homebanners_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->msmsl_homebanner_id->CurrentValue == "")
			$this->Page_Terminate("msmsl_homebannerslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->msmsl_homebanner_id->FldIsDetailKey)
			$this->msmsl_homebanner_id->setFormValue($objForm->GetValue("x_msmsl_homebanner_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->msmsl_homebanner_id->CurrentValue = $this->msmsl_homebanner_id->FormValue;
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->msmsl_homebanner_pic);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// msmsl_homebanner_top_title
			$this->msmsl_homebanner_top_title->SetDbValueDef($rsnew, $this->msmsl_homebanner_top_title->CurrentValue, "", $this->msmsl_homebanner_top_title->ReadOnly);

			// msmsl_homebanner_main_title
			$this->msmsl_homebanner_main_title->SetDbValueDef($rsnew, $this->msmsl_homebanner_main_title->CurrentValue, "", $this->msmsl_homebanner_main_title->ReadOnly);

			// msmsl_homebanner_low_title
			$this->msmsl_homebanner_low_title->SetDbValueDef($rsnew, $this->msmsl_homebanner_low_title->CurrentValue, "", $this->msmsl_homebanner_low_title->ReadOnly);

			// msmsl_homebanner_pic
			if (!($this->msmsl_homebanner_pic->ReadOnly) && !$this->msmsl_homebanner_pic->Upload->KeepFile) {
				$this->msmsl_homebanner_pic->Upload->DbValue = $rsold['msmsl_homebanner_pic']; // Get original value
			...
			// msmsl_homebanner_button_text
			$this->msmsl_homebanner_button_text->SetDbValueDef($rsnew, $this->msmsl_homebanner_button_text->CurrentValue, "", $this->msmsl_homebanner_button_text->ReadOnly);

			// msmsl_homebanner_banner_link
			$this->msmsl_homebanner_banner_link->SetDbValueDef($rsnew, $this->msmsl_homebanner_banner_link->CurrentValue, NULL, $this->msmsl_homebanner_banner_link->ReadOnly);
...
// Create page object
if (!isset($msmsl_homebanners_edit)) $msmsl_homebanners_edit = new cmsmsl_homebanners_edit();

// Page init
$msmsl_homebanners_edit->Page_Init();

// Page main
$msmsl_homebanners_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$msmsl_homebanners_edit->Page_Render();
?>
...
var CurrentForm = fmsmsl_homebannersedit = new ew_Form("fmsmsl_homebannersedit", "edit");

// Validate form
fmsmsl_homebannersedit.Validate = function() {
...
fmsmsl_homebannersedit.Form_CustomValidate = 
...
fmsmsl_homebannersedit.ValidateRequired = true;
<?php } else { ?>
fmsmsl_homebannersedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmsmsl_homebannersedit.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmsmsl_homebannersedit.Lists["x_status"].Options = <?php echo json_encode($msmsl_homebanners->status->Options()) ?>;
...
<?php $msmsl_homebanners_edit->ShowPageHeader(); ?>
<?php
$msmsl_homebanners_edit->ShowMessage();
?>
<form name="fmsmsl_homebannersedit" id="fmsmsl_homebannersedit" class="<?php echo $msmsl_homebanners_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($msmsl_homebanners_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $msmsl_homebanners_edit->Token ?>">
<?php } ?>
...
<?php if ($msmsl_homebanners->msmsl_homebanner_id->Visible) { // msmsl_homebanner_id ?>
	<div id="r_msmsl_homebanner_id" class="form-group">
		<label id="elh_msmsl_homebanners_msmsl_homebanner_id" class="col-sm-2 control-label ewLabel"><?php echo $msmsl_homebanners->msmsl_homebanner_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $msmsl_homebanners->msmsl_homebanner_id->CellAttributes() ?>>
<span id="el_msmsl_homebanners_msmsl_homebanner_id">
<span<?php echo $msmsl_homebanners->msmsl_homebanner_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $msmsl_homebanners->msmsl_homebanner_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="msmsl_homebanners" data-field="x_msmsl_homebanner_id" name="x_msmsl_homebanner_id" id="x_msmsl_homebanner_id" value="<?php echo ew_HtmlEncode($msmsl_homebanners->msmsl_homebanner_id->CurrentValue) ?>">
<?php echo $msmsl_homebanners->msmsl_homebanner_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if (@$_POST["fa_x_msmsl_homebanner_pic"] == "0") { ?>
<input type="hidden" name="fa_x_msmsl_homebanner_pic" id= "fa_x_msmsl_homebanner_pic" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_msmsl_homebanner_pic" id= "fa_x_msmsl_homebanner_pic" value="1">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $msmsl_homebanners_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fmsmsl_homebannersedit.Init();
</script>
<?php
$msmsl_homebanners_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$msmsl_homebanners_edit->Page_Terminate();
?>
