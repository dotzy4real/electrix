...
$msmsl_projects_delete = NULL; // Initialize page object first

class cmsmsl_projects_delete extends cmsmsl_projects {
	...
	// Page object name
	var $PageObjName = 'msmsl_projects_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("msmsl_projectslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in msmsl_projects class, msmsl_projectsinfo.php
...
// Create page object
if (!isset($msmsl_projects_delete)) $msmsl_projects_delete = new cmsmsl_projects_delete();

// Page init
$msmsl_projects_delete->Page_Init();

// Page main
$msmsl_projects_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$msmsl_projects_delete->Page_Render();
?>
...
var CurrentForm = fmsmsl_projectsdelete = new ew_Form("fmsmsl_projectsdelete", "delete");

// Form_CustomValidate event
fmsmsl_projectsdelete.Form_CustomValidate = 
...
fmsmsl_projectsdelete.ValidateRequired = true;
<?php } else { ?>
fmsmsl_projectsdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmsmsl_projectsdelete.Lists["x_msmsl_project_category_id"] = {"LinkField":"x_msmsl_project_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_msmsl_project_category_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmsmsl_projectsdelete.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmsmsl_projectsdelete.Lists["x_status"].Options = <?php echo json_encode($msmsl_projects->status->Options()) ?>;
...
if ($msmsl_projects_delete->Recordset = $msmsl_projects_delete->LoadRecordset())
	$msmsl_projects_deleteTotalRecs = $msmsl_projects_delete->Recordset->RecordCount(); // Get record count
if ($msmsl_projects_deleteTotalRecs <= 0) { // No record found, exit
	if ($msmsl_projects_delete->Recordset)
		$msmsl_projects_delete->Recordset->Close();
	$msmsl_projects_delete->Page_Terminate("msmsl_projectslist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $msmsl_projects_delete->ShowPageHeader(); ?>
<?php
$msmsl_projects_delete->ShowMessage();
?>
<form name="fmsmsl_projectsdelete" id="fmsmsl_projectsdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($msmsl_projects_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $msmsl_projects_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="msmsl_projects">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($msmsl_projects_delete->RecKeys as $key) { ?>
...
<?php if ($msmsl_projects->msmsl_project_id->Visible) { // msmsl_project_id ?>
		<th><span id="elh_msmsl_projects_msmsl_project_id" class="msmsl_projects_msmsl_project_id"><?php echo $msmsl_projects->msmsl_project_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($msmsl_projects->msmsl_project_pic->Visible) { // msmsl_project_pic ?>
		<th><span id="elh_msmsl_projects_msmsl_project_pic" class="msmsl_projects_msmsl_project_pic"><?php echo $msmsl_projects->msmsl_project_pic->FldCaption() ?></span></th>
<?php } ?>
<?php if ($msmsl_projects->msmsl_project_title->Visible) { // msmsl_project_title ?>
		<th><span id="elh_msmsl_projects_msmsl_project_title" class="msmsl_projects_msmsl_project_title"><?php echo $msmsl_projects->msmsl_project_title->FldCaption() ?></span></th>
<?php } ?>
<?php if ($msmsl_projects->msmsl_project_category_id->Visible) { // msmsl_project_category_id ?>
		<th><span id="elh_msmsl_projects_msmsl_project_category_id" class="msmsl_projects_msmsl_project_category_id"><?php echo $msmsl_projects->msmsl_project_category_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($msmsl_projects->sort_order->Visible) { // sort_order ?>
		<th><span id="elh_msmsl_projects_sort_order" class="msmsl_projects_sort_order"><?php echo $msmsl_projects->sort_order->FldCaption() ?></span></th>
<?php } ?>
<?php if ($msmsl_projects->status->Visible) { // status ?>
		<th><span id="elh_msmsl_projects_status" class="msmsl_projects_status"><?php echo $msmsl_projects->status->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$msmsl_projects_delete->RecCnt = 0;
$i = 0;
while (!$msmsl_projects_delete->Recordset->EOF) {
	$msmsl_projects_delete->RecCnt++;
	$msmsl_projects_delete->RowCnt++;

	// Set row properties
	$msmsl_projects->ResetAttrs();
	$msmsl_projects->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$msmsl_projects_delete->LoadRowValues($msmsl_projects_delete->Recordset);

	// Render row
	$msmsl_projects_delete->RenderRow();
?>
	<tr<?php echo $msmsl_projects->RowAttributes() ?>>
<?php if ($msmsl_projects->msmsl_project_id->Visible) { // msmsl_project_id ?>
		<td<?php echo $msmsl_projects->msmsl_project_id->CellAttributes() ?>>
<span id="el<?php echo $msmsl_projects_delete->RowCnt ?>_msmsl_projects_msmsl_project_id" class="msmsl_projects_msmsl_project_id">
...
<?php } ?>
<?php if ($msmsl_projects->msmsl_project_pic->Visible) { // msmsl_project_pic ?>
		<td<?php echo $msmsl_projects->msmsl_project_pic->CellAttributes() ?>>
<span id="el<?php echo $msmsl_projects_delete->RowCnt ?>_msmsl_projects_msmsl_project_pic" class="msmsl_projects_msmsl_project_pic">
<span>
...
<?php } ?>
<?php if ($msmsl_projects->msmsl_project_title->Visible) { // msmsl_project_title ?>
		<td<?php echo $msmsl_projects->msmsl_project_title->CellAttributes() ?>>
<span id="el<?php echo $msmsl_projects_delete->RowCnt ?>_msmsl_projects_msmsl_project_title" class="msmsl_projects_msmsl_project_title">
...
<?php } ?>
<?php if ($msmsl_projects->msmsl_project_category_id->Visible) { // msmsl_project_category_id ?>
		<td<?php echo $msmsl_projects->msmsl_project_category_id->CellAttributes() ?>>
<span id="el<?php echo $msmsl_projects_delete->RowCnt ?>_msmsl_projects_msmsl_project_category_id" class="msmsl_projects_msmsl_project_category_id">
...
<?php } ?>
<?php if ($msmsl_projects->sort_order->Visible) { // sort_order ?>
		<td<?php echo $msmsl_projects->sort_order->CellAttributes() ?>>
<span id="el<?php echo $msmsl_projects_delete->RowCnt ?>_msmsl_projects_sort_order" class="msmsl_projects_sort_order">
...
<?php } ?>
<?php if ($msmsl_projects->status->Visible) { // status ?>
		<td<?php echo $msmsl_projects->status->CellAttributes() ?>>
<span id="el<?php echo $msmsl_projects_delete->RowCnt ?>_msmsl_projects_status" class="msmsl_projects_status">
...
<?php
	$msmsl_projects_delete->Recordset->MoveNext();
}
$msmsl_projects_delete->Recordset->Close();
?>
</tbody>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $msmsl_projects_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fmsmsl_projectsdelete.Init();
</script>
<?php
$msmsl_projects_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$msmsl_projects_delete->Page_Terminate();
?>
