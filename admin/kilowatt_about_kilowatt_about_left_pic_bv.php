...
$kilowatt_about_kilowatt_about_left_pic_blobview = NULL; // Initialize page object first

class ckilowatt_about_kilowatt_about_left_pic_blobview extends ckilowatt_about {
	...
	// Page object name
	var $PageObjName = 'kilowatt_about_kilowatt_about_left_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('kilowatt_about', 'x_kilowatt_about_left_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('kilowatt_about_left_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('kilowatt_about_left_pic')));
				if (trim(strval($this->Recordset->fields('kilowatt_about_left_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('kilowatt_about_left_pic') . "\"");
				}
...
// Create page object
if (!isset($kilowatt_about_kilowatt_about_left_pic_blobview)) $kilowatt_about_kilowatt_about_left_pic_blobview = new ckilowatt_about_kilowatt_about_left_pic_blobview();

// Page init
$kilowatt_about_kilowatt_about_left_pic_blobview->Page_Init();

// Page main
$kilowatt_about_kilowatt_about_left_pic_blobview->Page_Main();
?>
<?php
$kilowatt_about_kilowatt_about_left_pic_blobview->Page_Terminate();
?>
