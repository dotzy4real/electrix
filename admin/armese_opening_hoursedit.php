...
$armese_opening_hours_edit = NULL; // Initialize page object first

class carmese_opening_hours_edit extends carmese_opening_hours {
	...
	// Page object name
	var $PageObjName = 'armese_opening_hours_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->armese_opening_hour_id->CurrentValue == "")
			$this->Page_Terminate("armese_opening_hourslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->armese_opening_hour_id->FldIsDetailKey)
			$this->armese_opening_hour_id->setFormValue($objForm->GetValue("x_armese_opening_hour_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->armese_opening_hour_id->CurrentValue = $this->armese_opening_hour_id->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// armese_opening_hour_name
			$this->armese_opening_hour_name->SetDbValueDef($rsnew, $this->armese_opening_hour_name->CurrentValue, "", $this->armese_opening_hour_name->ReadOnly);

			// armese_opening_hour_time
			$this->armese_opening_hour_time->SetDbValueDef($rsnew, $this->armese_opening_hour_time->CurrentValue, "", $this->armese_opening_hour_time->ReadOnly);
...
// Create page object
if (!isset($armese_opening_hours_edit)) $armese_opening_hours_edit = new carmese_opening_hours_edit();

// Page init
$armese_opening_hours_edit->Page_Init();

// Page main
$armese_opening_hours_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$armese_opening_hours_edit->Page_Render();
?>
...
var CurrentForm = farmese_opening_hoursedit = new ew_Form("farmese_opening_hoursedit", "edit");

// Validate form
farmese_opening_hoursedit.Validate = function() {
...
farmese_opening_hoursedit.Form_CustomValidate = 
...
farmese_opening_hoursedit.ValidateRequired = true;
<?php } else { ?>
farmese_opening_hoursedit.ValidateRequired = false; 
<?php } ?>
...
<?php $armese_opening_hours_edit->ShowPageHeader(); ?>
<?php
$armese_opening_hours_edit->ShowMessage();
?>
<form name="farmese_opening_hoursedit" id="farmese_opening_hoursedit" class="<?php echo $armese_opening_hours_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($armese_opening_hours_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $armese_opening_hours_edit->Token ?>">
<?php } ?>
...
<?php if ($armese_opening_hours->armese_opening_hour_id->Visible) { // armese_opening_hour_id ?>
	<div id="r_armese_opening_hour_id" class="form-group">
		<label id="elh_armese_opening_hours_armese_opening_hour_id" class="col-sm-2 control-label ewLabel"><?php echo $armese_opening_hours->armese_opening_hour_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $armese_opening_hours->armese_opening_hour_id->CellAttributes() ?>>
...
<p class="form-control-static"><?php echo $armese_opening_hours->armese_opening_hour_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="armese_opening_hours" data-field="x_armese_opening_hour_id" name="x_armese_opening_hour_id" id="x_armese_opening_hour_id" value="<?php echo ew_HtmlEncode($armese_opening_hours->armese_opening_hour_id->CurrentValue) ?>">
<?php echo $armese_opening_hours->armese_opening_hour_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $armese_opening_hours_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
farmese_opening_hoursedit.Init();
</script>
<?php
$armese_opening_hours_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$armese_opening_hours_edit->Page_Terminate();
?>
