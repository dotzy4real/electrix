...
$blog_categories_add = NULL; // Initialize page object first

class cblog_categories_add extends cblog_categories {
	...
	// Page object name
	var $PageObjName = 'blog_categories_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["blog_category_id"] != "") {
				...
				$this->setKey("blog_category_id", $this->blog_category_id->CurrentValue); // Set up key
			} else {
				$this->setKey("blog_category_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "blog_categoriesview.php")
	...
	function LoadDefaultValues() {
		$this->blog_category_name->CurrentValue = NULL;
		$this->blog_category_name->OldValue = $this->blog_category_name->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// blog_category_name
		$this->blog_category_name->SetDbValueDef($rsnew, $this->blog_category_name->CurrentValue, "", FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->blog_category_id->setDbValue($conn->Insert_ID());
				$rsnew['blog_category_id'] = $this->blog_category_id->DbValue;
			}
		} else {
...
// Create page object
if (!isset($blog_categories_add)) $blog_categories_add = new cblog_categories_add();

// Page init
$blog_categories_add->Page_Init();

// Page main
$blog_categories_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$blog_categories_add->Page_Render();
?>
...
var CurrentForm = fblog_categoriesadd = new ew_Form("fblog_categoriesadd", "add");

// Validate form
fblog_categoriesadd.Validate = function() {
...
fblog_categoriesadd.Form_CustomValidate = 
...
fblog_categoriesadd.ValidateRequired = true;
<?php } else { ?>
fblog_categoriesadd.ValidateRequired = false; 
<?php } ?>
...
<?php $blog_categories_add->ShowPageHeader(); ?>
<?php
$blog_categories_add->ShowMessage();
?>
<form name="fblog_categoriesadd" id="fblog_categoriesadd" class="<?php echo $blog_categories_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($blog_categories_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $blog_categories_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $blog_categories_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fblog_categoriesadd.Init();
</script>
<?php
$blog_categories_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$blog_categories_add->Page_Terminate();
?>
