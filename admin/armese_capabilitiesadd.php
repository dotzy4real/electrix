...
$armese_capabilities_add = NULL; // Initialize page object first

class carmese_capabilities_add extends carmese_capabilities {
	...
	// Page object name
	var $PageObjName = 'armese_capabilities_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["armese_capability_id"] != "") {
				...
				$this->setKey("armese_capability_id", $this->armese_capability_id->CurrentValue); // Set up key
			} else {
				$this->setKey("armese_capability_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "armese_capabilitiesview.php")
	...
	function LoadDefaultValues() {
		$this->armese_capability_title->CurrentValue = NULL;
		$this->armese_capability_title->OldValue = $this->armese_capability_title->CurrentValue;
		$this->armese_capability_content->CurrentValue = NULL;
		$this->armese_capability_content->OldValue = $this->armese_capability_content->CurrentValue;
		$this->armese_capability_icon->CurrentValue = NULL;
		$this->armese_capability_icon->OldValue = $this->armese_capability_icon->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// armese_capability_title
		$this->armese_capability_title->SetDbValueDef($rsnew, $this->armese_capability_title->CurrentValue, "", FALSE);

		// armese_capability_content
		$this->armese_capability_content->SetDbValueDef($rsnew, $this->armese_capability_content->CurrentValue, "", FALSE);

		// armese_capability_icon
		$this->armese_capability_icon->SetDbValueDef($rsnew, $this->armese_capability_icon->CurrentValue, "", FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->armese_capability_id->setDbValue($conn->Insert_ID());
				$rsnew['armese_capability_id'] = $this->armese_capability_id->DbValue;
			}
		} else {
...
// Create page object
if (!isset($armese_capabilities_add)) $armese_capabilities_add = new carmese_capabilities_add();

// Page init
$armese_capabilities_add->Page_Init();

// Page main
$armese_capabilities_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$armese_capabilities_add->Page_Render();
?>
...
var CurrentForm = farmese_capabilitiesadd = new ew_Form("farmese_capabilitiesadd", "add");

// Validate form
farmese_capabilitiesadd.Validate = function() {
...
farmese_capabilitiesadd.Form_CustomValidate = 
...
farmese_capabilitiesadd.ValidateRequired = true;
<?php } else { ?>
farmese_capabilitiesadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
farmese_capabilitiesadd.Lists["x_armese_capability_icon"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
farmese_capabilitiesadd.Lists["x_armese_capability_icon"].Options = <?php echo json_encode($armese_capabilities->armese_capability_icon->Options()) ?>;
farmese_capabilitiesadd.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
farmese_capabilitiesadd.Lists["x_status"].Options = <?php echo json_encode($armese_capabilities->status->Options()) ?>;
...
<?php $armese_capabilities_add->ShowPageHeader(); ?>
<?php
$armese_capabilities_add->ShowMessage();
?>
<form name="farmese_capabilitiesadd" id="farmese_capabilitiesadd" class="<?php echo $armese_capabilities_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($armese_capabilities_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $armese_capabilities_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $armese_capabilities_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
farmese_capabilitiesadd.Init();
</script>
<?php
$armese_capabilities_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$armese_capabilities_add->Page_Terminate();
?>
