...
$kilowatt_homebanners_edit = NULL; // Initialize page object first

class ckilowatt_homebanners_edit extends ckilowatt_homebanners {
	...
	// Page object name
	var $PageObjName = 'kilowatt_homebanners_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->kilowatt_homebanner_id->CurrentValue == "")
			$this->Page_Terminate("kilowatt_homebannerslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->kilowatt_homebanner_id->FldIsDetailKey)
			$this->kilowatt_homebanner_id->setFormValue($objForm->GetValue("x_kilowatt_homebanner_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->kilowatt_homebanner_id->CurrentValue = $this->kilowatt_homebanner_id->FormValue;
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->kilowatt_homebanner_pic);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// kilowatt_homebanner_icontitle
			$this->kilowatt_homebanner_icontitle->SetDbValueDef($rsnew, $this->kilowatt_homebanner_icontitle->CurrentValue, "", $this->kilowatt_homebanner_icontitle->ReadOnly);

			// kilowatt_homebanner_maintitle
			$this->kilowatt_homebanner_maintitle->SetDbValueDef($rsnew, $this->kilowatt_homebanner_maintitle->CurrentValue, "", $this->kilowatt_homebanner_maintitle->ReadOnly);

			// kilowatt_homebanner_subtitle
			$this->kilowatt_homebanner_subtitle->SetDbValueDef($rsnew, $this->kilowatt_homebanner_subtitle->CurrentValue, "", $this->kilowatt_homebanner_subtitle->ReadOnly);

			// kilowatt_homebanner_pic
			if (!($this->kilowatt_homebanner_pic->ReadOnly) && !$this->kilowatt_homebanner_pic->Upload->KeepFile) {
				$this->kilowatt_homebanner_pic->Upload->DbValue = $rsold['kilowatt_homebanner_pic']; // Get original value
			...
			// kilowatt_homebanner_buttontext
			$this->kilowatt_homebanner_buttontext->SetDbValueDef($rsnew, $this->kilowatt_homebanner_buttontext->CurrentValue, "", $this->kilowatt_homebanner_buttontext->ReadOnly);

			// kilowatt_homebanner_buttonlink
			$this->kilowatt_homebanner_buttonlink->SetDbValueDef($rsnew, $this->kilowatt_homebanner_buttonlink->CurrentValue, NULL, $this->kilowatt_homebanner_buttonlink->ReadOnly);
...
// Create page object
if (!isset($kilowatt_homebanners_edit)) $kilowatt_homebanners_edit = new ckilowatt_homebanners_edit();

// Page init
$kilowatt_homebanners_edit->Page_Init();

// Page main
$kilowatt_homebanners_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kilowatt_homebanners_edit->Page_Render();
?>
...
var CurrentForm = fkilowatt_homebannersedit = new ew_Form("fkilowatt_homebannersedit", "edit");

// Validate form
fkilowatt_homebannersedit.Validate = function() {
...
fkilowatt_homebannersedit.Form_CustomValidate = 
...
fkilowatt_homebannersedit.ValidateRequired = true;
<?php } else { ?>
fkilowatt_homebannersedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkilowatt_homebannersedit.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_homebannersedit.Lists["x_status"].Options = <?php echo json_encode($kilowatt_homebanners->status->Options()) ?>;
...
<?php $kilowatt_homebanners_edit->ShowPageHeader(); ?>
<?php
$kilowatt_homebanners_edit->ShowMessage();
?>
<form name="fkilowatt_homebannersedit" id="fkilowatt_homebannersedit" class="<?php echo $kilowatt_homebanners_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kilowatt_homebanners_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kilowatt_homebanners_edit->Token ?>">
<?php } ?>
...
<?php if ($kilowatt_homebanners->kilowatt_homebanner_id->Visible) { // kilowatt_homebanner_id ?>
	<div id="r_kilowatt_homebanner_id" class="form-group">
		<label id="elh_kilowatt_homebanners_kilowatt_homebanner_id" class="col-sm-2 control-label ewLabel"><?php echo $kilowatt_homebanners->kilowatt_homebanner_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $kilowatt_homebanners->kilowatt_homebanner_id->CellAttributes() ?>>
...
<p class="form-control-static"><?php echo $kilowatt_homebanners->kilowatt_homebanner_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="kilowatt_homebanners" data-field="x_kilowatt_homebanner_id" name="x_kilowatt_homebanner_id" id="x_kilowatt_homebanner_id" value="<?php echo ew_HtmlEncode($kilowatt_homebanners->kilowatt_homebanner_id->CurrentValue) ?>">
<?php echo $kilowatt_homebanners->kilowatt_homebanner_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if (@$_POST["fa_x_kilowatt_homebanner_pic"] == "0") { ?>
<input type="hidden" name="fa_x_kilowatt_homebanner_pic" id= "fa_x_kilowatt_homebanner_pic" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_kilowatt_homebanner_pic" id= "fa_x_kilowatt_homebanner_pic" value="1">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kilowatt_homebanners_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fkilowatt_homebannersedit.Init();
</script>
<?php
$kilowatt_homebanners_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kilowatt_homebanners_edit->Page_Terminate();
?>
