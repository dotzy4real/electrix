...
$kilowatt_services_view = NULL; // Initialize page object first

class ckilowatt_services_view extends ckilowatt_services {
	...
	// Page object name
	var $PageObjName = 'kilowatt_services_view';
	...
	function __construct() {
		...
		if (@$_GET["kilowatt_service_id"] <> "") {
			$this->RecKey["kilowatt_service_id"] = $_GET["kilowatt_service_id"];
			$KeyUrl .= "&amp;kilowatt_service_id=" . urlencode($this->RecKey["kilowatt_service_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["kilowatt_service_id"] <> "") {
				...
				$this->RecKey["kilowatt_service_id"] = $this->kilowatt_service_id->QueryStringValue;
			} elseif (@$_POST["kilowatt_service_id"] <> "") {
				$this->kilowatt_service_id->setFormValue($_POST["kilowatt_service_id"]);
				$this->RecKey["kilowatt_service_id"] = $this->kilowatt_service_id->FormValue;
			} else {
				$sReturnUrl = "kilowatt_serviceslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "kilowatt_serviceslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "kilowatt_serviceslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($kilowatt_services_view)) $kilowatt_services_view = new ckilowatt_services_view();

// Page init
$kilowatt_services_view->Page_Init();

// Page main
$kilowatt_services_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kilowatt_services_view->Page_Render();
?>
...
var CurrentForm = fkilowatt_servicesview = new ew_Form("fkilowatt_servicesview", "view");

// Form_CustomValidate event
fkilowatt_servicesview.Form_CustomValidate = 
...
fkilowatt_servicesview.ValidateRequired = true;
<?php } else { ?>
fkilowatt_servicesview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkilowatt_servicesview.Lists["x_kilowatt_service_category_id"] = {"LinkField":"x_kilowatt_service_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_kilowatt_service_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_servicesview.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_servicesview.Lists["x_status"].Options = <?php echo json_encode($kilowatt_services->status->Options()) ?>;
...
<?php $kilowatt_services_view->ExportOptions->Render("body") ?>
<?php
	foreach ($kilowatt_services_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $kilowatt_services_view->ShowPageHeader(); ?>
<?php
$kilowatt_services_view->ShowMessage();
?>
<form name="fkilowatt_servicesview" id="fkilowatt_servicesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kilowatt_services_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kilowatt_services_view->Token ?>">
<?php } ?>
...
<?php if ($kilowatt_services->kilowatt_service_id->Visible) { // kilowatt_service_id ?>
	<tr id="r_kilowatt_service_id">
		<td><span id="elh_kilowatt_services_kilowatt_service_id"><?php echo $kilowatt_services->kilowatt_service_id->FldCaption() ?></span></td>
...
<?php echo $kilowatt_services->kilowatt_service_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kilowatt_services->kilowatt_service_title->Visible) { // kilowatt_service_title ?>
	<tr id="r_kilowatt_service_title">
		<td><span id="elh_kilowatt_services_kilowatt_service_title"><?php echo $kilowatt_services->kilowatt_service_title->FldCaption() ?></span></td>
...
<?php echo $kilowatt_services->kilowatt_service_title->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kilowatt_services->kilowatt_service_pic->Visible) { // kilowatt_service_pic ?>
	<tr id="r_kilowatt_service_pic">
		<td><span id="elh_kilowatt_services_kilowatt_service_pic"><?php echo $kilowatt_services->kilowatt_service_pic->FldCaption() ?></span></td>
...
<span>
<?php echo ew_GetFileViewTag($kilowatt_services->kilowatt_service_pic, $kilowatt_services->kilowatt_service_pic->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($kilowatt_services->kilowatt_service_category_id->Visible) { // kilowatt_service_category_id ?>
	<tr id="r_kilowatt_service_category_id">
		<td><span id="elh_kilowatt_services_kilowatt_service_category_id"><?php echo $kilowatt_services->kilowatt_service_category_id->FldCaption() ?></span></td>
...
<span>
<?php if (!ew_EmptyStr($kilowatt_services->kilowatt_service_category_id->ViewValue)) { ?><img src="<?php echo $kilowatt_services->kilowatt_service_category_id->ViewValue ?>" alt=""<?php echo $kilowatt_services->kilowatt_service_category_id->ViewAttributes() ?>><?php } ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kilowatt_services->sort_order->Visible) { // sort_order ?>
	<tr id="r_sort_order">
		<td><span id="elh_kilowatt_services_sort_order"><?php echo $kilowatt_services->sort_order->FldCaption() ?></span></td>
...
<?php echo $kilowatt_services->sort_order->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kilowatt_services->status->Visible) { // status ?>
	<tr id="r_status">
		<td><span id="elh_kilowatt_services_status"><?php echo $kilowatt_services->status->FldCaption() ?></span></td>
...
<?php echo $kilowatt_services->status->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fkilowatt_servicesview.Init();
</script>
<?php
$kilowatt_services_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kilowatt_services_view->Page_Terminate();
?>
