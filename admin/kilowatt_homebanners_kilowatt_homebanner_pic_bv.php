...
$kilowatt_homebanners_kilowatt_homebanner_pic_blobview = NULL; // Initialize page object first

class ckilowatt_homebanners_kilowatt_homebanner_pic_blobview extends ckilowatt_homebanners {
	...
	// Page object name
	var $PageObjName = 'kilowatt_homebanners_kilowatt_homebanner_pic_blobview';
	...
	function Page_Main() {
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('kilowatt_homebanner_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('kilowatt_homebanner_pic')));
				if (trim(strval($this->Recordset->fields('kilowatt_homebanner_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('kilowatt_homebanner_pic') . "\"");
				}
...
// Create page object
if (!isset($kilowatt_homebanners_kilowatt_homebanner_pic_blobview)) $kilowatt_homebanners_kilowatt_homebanner_pic_blobview = new ckilowatt_homebanners_kilowatt_homebanner_pic_blobview();

// Page init
$kilowatt_homebanners_kilowatt_homebanner_pic_blobview->Page_Init();

// Page main
$kilowatt_homebanners_kilowatt_homebanner_pic_blobview->Page_Main();
?>
<?php
$kilowatt_homebanners_kilowatt_homebanner_pic_blobview->Page_Terminate();
?>
