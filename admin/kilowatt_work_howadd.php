...
$kilowatt_work_how_add = NULL; // Initialize page object first

class ckilowatt_work_how_add extends ckilowatt_work_how {
	...
	// Page object name
	var $PageObjName = 'kilowatt_work_how_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["kilowatt_work_how_id"] != "") {
				...
				$this->setKey("kilowatt_work_how_id", $this->kilowatt_work_how_id->CurrentValue); // Set up key
			} else {
				$this->setKey("kilowatt_work_how_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "kilowatt_work_howview.php")
	...
	function LoadDefaultValues() {
		$this->kilowatt_work_how_title->CurrentValue = NULL;
		$this->kilowatt_work_how_title->OldValue = $this->kilowatt_work_how_title->CurrentValue;
		$this->kilowatt_work_how_description->CurrentValue = NULL;
		$this->kilowatt_work_how_description->OldValue = $this->kilowatt_work_how_description->CurrentValue;
		$this->kilowatt_work_how_icon->CurrentValue = NULL;
		$this->kilowatt_work_how_icon->OldValue = $this->kilowatt_work_how_icon->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// kilowatt_work_how_title
		$this->kilowatt_work_how_title->SetDbValueDef($rsnew, $this->kilowatt_work_how_title->CurrentValue, "", FALSE);

		// kilowatt_work_how_description
		$this->kilowatt_work_how_description->SetDbValueDef($rsnew, $this->kilowatt_work_how_description->CurrentValue, "", FALSE);

		// kilowatt_work_how_icon
		$this->kilowatt_work_how_icon->SetDbValueDef($rsnew, $this->kilowatt_work_how_icon->CurrentValue, "", FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->kilowatt_work_how_id->setDbValue($conn->Insert_ID());
				$rsnew['kilowatt_work_how_id'] = $this->kilowatt_work_how_id->DbValue;
			}
		} else {
...
// Create page object
if (!isset($kilowatt_work_how_add)) $kilowatt_work_how_add = new ckilowatt_work_how_add();

// Page init
$kilowatt_work_how_add->Page_Init();

// Page main
$kilowatt_work_how_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kilowatt_work_how_add->Page_Render();
?>
...
var CurrentForm = fkilowatt_work_howadd = new ew_Form("fkilowatt_work_howadd", "add");

// Validate form
fkilowatt_work_howadd.Validate = function() {
...
fkilowatt_work_howadd.Form_CustomValidate = 
...
fkilowatt_work_howadd.ValidateRequired = true;
<?php } else { ?>
fkilowatt_work_howadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkilowatt_work_howadd.Lists["x_kilowatt_work_how_icon"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_work_howadd.Lists["x_kilowatt_work_how_icon"].Options = <?php echo json_encode($kilowatt_work_how->kilowatt_work_how_icon->Options()) ?>;
fkilowatt_work_howadd.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_work_howadd.Lists["x_status"].Options = <?php echo json_encode($kilowatt_work_how->status->Options()) ?>;
...
<?php $kilowatt_work_how_add->ShowPageHeader(); ?>
<?php
$kilowatt_work_how_add->ShowMessage();
?>
<form name="fkilowatt_work_howadd" id="fkilowatt_work_howadd" class="<?php echo $kilowatt_work_how_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kilowatt_work_how_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kilowatt_work_how_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kilowatt_work_how_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fkilowatt_work_howadd.Init();
</script>
<?php
$kilowatt_work_how_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kilowatt_work_how_add->Page_Terminate();
?>
