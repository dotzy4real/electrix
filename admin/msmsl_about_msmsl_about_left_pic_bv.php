...
$msmsl_about_msmsl_about_left_pic_blobview = NULL; // Initialize page object first

class cmsmsl_about_msmsl_about_left_pic_blobview extends cmsmsl_about {
	...
	// Page object name
	var $PageObjName = 'msmsl_about_msmsl_about_left_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('msmsl_about', 'x_msmsl_about_left_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('msmsl_about_left_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('msmsl_about_left_pic')));
				if (trim(strval($this->Recordset->fields('msmsl_about_left_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('msmsl_about_left_pic') . "\"");
				}
...
// Create page object
if (!isset($msmsl_about_msmsl_about_left_pic_blobview)) $msmsl_about_msmsl_about_left_pic_blobview = new cmsmsl_about_msmsl_about_left_pic_blobview();

// Page init
$msmsl_about_msmsl_about_left_pic_blobview->Page_Init();

// Page main
$msmsl_about_msmsl_about_left_pic_blobview->Page_Main();
?>
<?php
$msmsl_about_msmsl_about_left_pic_blobview->Page_Terminate();
?>
