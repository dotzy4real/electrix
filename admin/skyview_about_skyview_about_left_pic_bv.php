...
$skyview_about_skyview_about_left_pic_blobview = NULL; // Initialize page object first

class cskyview_about_skyview_about_left_pic_blobview extends cskyview_about {
	...
	// Page object name
	var $PageObjName = 'skyview_about_skyview_about_left_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('skyview_about', 'x_skyview_about_left_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('skyview_about_left_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('skyview_about_left_pic')));
				if (trim(strval($this->Recordset->fields('skyview_about_left_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('skyview_about_left_pic') . "\"");
				}
...
// Create page object
if (!isset($skyview_about_skyview_about_left_pic_blobview)) $skyview_about_skyview_about_left_pic_blobview = new cskyview_about_skyview_about_left_pic_blobview();

// Page init
$skyview_about_skyview_about_left_pic_blobview->Page_Init();

// Page main
$skyview_about_skyview_about_left_pic_blobview->Page_Main();
?>
<?php
$skyview_about_skyview_about_left_pic_blobview->Page_Terminate();
?>
