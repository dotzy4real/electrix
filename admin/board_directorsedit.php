...
$board_directors_edit = NULL; // Initialize page object first

class cboard_directors_edit extends cboard_directors {
	...
	// Page object name
	var $PageObjName = 'board_directors_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->board_director_id->CurrentValue == "")
			$this->Page_Terminate("board_directorslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->board_director_id->FldIsDetailKey)
			$this->board_director_id->setFormValue($objForm->GetValue("x_board_director_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->board_director_id->CurrentValue = $this->board_director_id->FormValue;
	...
	function RenderRow() {
		...
		$this->board_director_phone->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->board_director_pic);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// board_director_pic
			if (!($this->board_director_pic->ReadOnly) && !$this->board_director_pic->Upload->KeepFile) {
				$this->board_director_pic->Upload->DbValue = $rsold['board_director_pic']; // Get original value
			...
			// board_director_name
			$this->board_director_name->SetDbValueDef($rsnew, $this->board_director_name->CurrentValue, "", $this->board_director_name->ReadOnly);

			// board_director_designation
			$this->board_director_designation->SetDbValueDef($rsnew, $this->board_director_designation->CurrentValue, "", $this->board_director_designation->ReadOnly);

			// board_director_content
			$this->board_director_content->SetDbValueDef($rsnew, $this->board_director_content->CurrentValue, "", $this->board_director_content->ReadOnly);
			...
			// board_director_linkedin
			$this->board_director_linkedin->SetDbValueDef($rsnew, $this->board_director_linkedin->CurrentValue, NULL, $this->board_director_linkedin->ReadOnly);

			// board_director_facebook
			$this->board_director_facebook->SetDbValueDef($rsnew, $this->board_director_facebook->CurrentValue, NULL, $this->board_director_facebook->ReadOnly);

			// board_director_twitter
			$this->board_director_twitter->SetDbValueDef($rsnew, $this->board_director_twitter->CurrentValue, NULL, $this->board_director_twitter->ReadOnly);

			// board_director_email
			$this->board_director_email->SetDbValueDef($rsnew, $this->board_director_email->CurrentValue, "", $this->board_director_email->ReadOnly);

			// board_director_phone
			$this->board_director_phone->SetDbValueDef($rsnew, $this->board_director_phone->CurrentValue, "", $this->board_director_phone->ReadOnly);
...
// Create page object
if (!isset($board_directors_edit)) $board_directors_edit = new cboard_directors_edit();

// Page init
$board_directors_edit->Page_Init();

// Page main
$board_directors_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$board_directors_edit->Page_Render();
?>
...
var CurrentForm = fboard_directorsedit = new ew_Form("fboard_directorsedit", "edit");

// Validate form
fboard_directorsedit.Validate = function() {
...
fboard_directorsedit.Form_CustomValidate = 
...
fboard_directorsedit.ValidateRequired = true;
<?php } else { ?>
fboard_directorsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fboard_directorsedit.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fboard_directorsedit.Lists["x_status"].Options = <?php echo json_encode($board_directors->status->Options()) ?>;
...
<?php $board_directors_edit->ShowPageHeader(); ?>
<?php
$board_directors_edit->ShowMessage();
?>
<form name="fboard_directorsedit" id="fboard_directorsedit" class="<?php echo $board_directors_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($board_directors_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $board_directors_edit->Token ?>">
<?php } ?>
...
<?php if ($board_directors->board_director_id->Visible) { // board_director_id ?>
	<div id="r_board_director_id" class="form-group">
		<label id="elh_board_directors_board_director_id" class="col-sm-2 control-label ewLabel"><?php echo $board_directors->board_director_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $board_directors->board_director_id->CellAttributes() ?>>
<span id="el_board_directors_board_director_id">
<span<?php echo $board_directors->board_director_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $board_directors->board_director_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="board_directors" data-field="x_board_director_id" name="x_board_director_id" id="x_board_director_id" value="<?php echo ew_HtmlEncode($board_directors->board_director_id->CurrentValue) ?>">
<?php echo $board_directors->board_director_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if (@$_POST["fa_x_board_director_pic"] == "0") { ?>
<input type="hidden" name="fa_x_board_director_pic" id= "fa_x_board_director_pic" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_board_director_pic" id= "fa_x_board_director_pic" value="1">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fboard_directorsedit", "x_board_director_content", 35, 4, <?php echo ($board_directors->board_director_content->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $board_directors_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fboard_directorsedit.Init();
</script>
<?php
$board_directors_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$board_directors_edit->Page_Terminate();
?>
