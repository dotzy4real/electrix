...
class cprojects_add extends cprojects {
	...
	function LoadDefaultValues() {
		...
		$this->project_small_pic->Upload->DbValue = NULL;
		$this->project_small_pic->OldValue = $this->project_small_pic->Upload->DbValue;
		$this->project_small_pic->CurrentValue = NULL; // Clear file related field
		...
		$this->project_content->CurrentValue = NULL;
		$this->project_content->OldValue = $this->project_content->CurrentValue;
		$this->project_category_id->CurrentValue = NULL;
		$this->project_category_id->OldValue = $this->project_category_id->CurrentValue;
		$this->project_date->CurrentValue = NULL;
		$this->project_date->OldValue = $this->project_date->CurrentValue;
		$this->project_client->CurrentValue = NULL;
		$this->project_client->OldValue = $this->project_client->CurrentValue;
		$this->project_location->CurrentValue = NULL;
		$this->project_location->OldValue = $this->project_location->CurrentValue;
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->project_small_pic);
	...
	function AddRow($rsold = NULL) {
		...
		if (!$this->project_small_pic->Upload->KeepFile) {
			$this->project_small_pic->Upload->DbValue = ""; // No need to delete old file
		...
		// project_content
		$this->project_content->SetDbValueDef($rsnew, $this->project_content->CurrentValue, "", FALSE);

		// project_category_id
		$this->project_category_id->SetDbValueDef($rsnew, $this->project_category_id->CurrentValue, 0, FALSE);

		// project_date
		$this->project_date->SetDbValueDef($rsnew, $this->project_date->CurrentValue, "", FALSE);

		// project_client
		$this->project_client->SetDbValueDef($rsnew, $this->project_client->CurrentValue, "", FALSE);

		// project_location
		$this->project_location->SetDbValueDef($rsnew, $this->project_location->CurrentValue, "", FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->project_id->setDbValue($conn->Insert_ID());
				$rsnew['project_id'] = $this->project_id->DbValue;
				if (!$this->project_small_pic->Upload->KeepFile) {
					if (!ew_Empty($this->project_small_pic->Upload->Value)) {
...
fprojectsadd.Lists["x_project_category_id"] = {"LinkField":"x_project_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_project_category_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fprojectsadd.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fprojectsadd.Lists["x_status"].Options = <?php echo json_encode($projects->status->Options()) ?>;
...
<script type="text/javascript">
ew_CreateEditor("fprojectsadd", "x_project_content", 35, 4, <?php echo ($projects->project_content->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
