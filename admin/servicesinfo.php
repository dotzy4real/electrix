...
class cservices extends cTable {
	var $service_id;
	var $service_small_pic;
	var $service_large_pic;
	var $service_title;
	var $service_content;
	var $service_icon;
	var $sort_order;
	...
	function __construct() {
		...
		// service_id
		$this->service_id = new cField('services', 'services', 'x_service_id', 'service_id', '`service_id`', '`service_id`', 19, -1, FALSE, '`service_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->service_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['service_id'] = &$this->service_id;

		// service_small_pic
		$this->service_small_pic = new cField('services', 'services', 'x_service_small_pic', 'service_small_pic', '`service_small_pic`', '`service_small_pic`', 200, -1, TRUE, '`service_small_pic`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['service_small_pic'] = &$this->service_small_pic;

		// service_large_pic
		$this->service_large_pic = new cField('services', 'services', 'x_service_large_pic', 'service_large_pic', '`service_large_pic`', '`service_large_pic`', 200, -1, TRUE, '`service_large_pic`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['service_large_pic'] = &$this->service_large_pic;

		// service_title
		$this->service_title = new cField('services', 'services', 'x_service_title', 'service_title', '`service_title`', '`service_title`', 200, -1, FALSE, '`service_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['service_title'] = &$this->service_title;

		// service_content
		$this->service_content = new cField('services', 'services', 'x_service_content', 'service_content', '`service_content`', '`service_content`', 201, -1, FALSE, '`service_content`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['service_content'] = &$this->service_content;

		// service_icon
		$this->service_icon = new cField('services', 'services', 'x_service_icon', 'service_icon', '`service_icon`', '`service_icon`', 202, -1, FALSE, '`service_icon`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->service_icon->OptionCount = 50;
		$this->fields['service_icon'] = &$this->service_icon;

		// sort_order
		$this->sort_order = new cField('services', 'services', 'x_sort_order', 'sort_order', '`sort_order`', '`sort_order`', 3, -1, FALSE, '`sort_order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->sort_order->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sort_order'] = &$this->sort_order;

		// status
		$this->status = new cField('services', 'services', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('service_id', $rs))
				ew_AddFilter($where, ew_QuotedName('service_id', $this->DBID) . '=' . ew_QuotedValue($rs['service_id'], $this->service_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@service_id@", ew_AdjustSql($this->service_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "service_id:" . ew_VarToJson($this->service_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = $isPost ? ew_StripSlashes(@$_POST["service_id"]) : ew_StripSlashes(@$_GET["service_id"]); // service_id
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->service_title->EditValue = $this->service_title->CurrentValue;
		...
		$this->service_content->EditValue = $this->service_content->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->service_id->Exportable) $Doc->ExportCaption($this->service_id);
					if ($this->service_small_pic->Exportable) $Doc->ExportCaption($this->service_small_pic);
					if ($this->service_large_pic->Exportable) $Doc->ExportCaption($this->service_large_pic);
					if ($this->service_title->Exportable) $Doc->ExportCaption($this->service_title);
					if ($this->service_content->Exportable) $Doc->ExportCaption($this->service_content);
					if ($this->service_icon->Exportable) $Doc->ExportCaption($this->service_icon);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->service_id->Exportable) $Doc->ExportField($this->service_id);
						if ($this->service_small_pic->Exportable) $Doc->ExportField($this->service_small_pic);
						if ($this->service_large_pic->Exportable) $Doc->ExportField($this->service_large_pic);
						if ($this->service_title->Exportable) $Doc->ExportField($this->service_title);
						if ($this->service_content->Exportable) $Doc->ExportField($this->service_content);
						if ($this->service_icon->Exportable) $Doc->ExportField($this->service_icon);
