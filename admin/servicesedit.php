...
class cservices_edit extends cservices {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->service_id->CurrentValue == "")
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->service_small_pic);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->service_large_pic);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// service_small_pic
			if (!($this->service_small_pic->ReadOnly) && !$this->service_small_pic->Upload->KeepFile) {
				$this->service_small_pic->Upload->DbValue = $rsold['service_small_pic']; // Get original value
			...
			// service_large_pic
			if (!($this->service_large_pic->ReadOnly) && !$this->service_large_pic->Upload->KeepFile) {
				$this->service_large_pic->Upload->DbValue = $rsold['service_large_pic']; // Get original value
			...
			// service_title
			$this->service_title->SetDbValueDef($rsnew, $this->service_title->CurrentValue, "", $this->service_title->ReadOnly);

			// service_content
			$this->service_content->SetDbValueDef($rsnew, $this->service_content->CurrentValue, "", $this->service_content->ReadOnly);

			// service_icon
			$this->service_icon->SetDbValueDef($rsnew, $this->service_icon->CurrentValue, "", $this->service_icon->ReadOnly);
...
fservicesedit.Lists["x_service_icon"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fservicesedit.Lists["x_service_icon"].Options = <?php echo json_encode($services->service_icon->Options()) ?>;
fservicesedit.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fservicesedit.Lists["x_status"].Options = <?php echo json_encode($services->status->Options()) ?>;
...
<?php if (@$_POST["fa_x_service_small_pic"] == "0") { ?>
<input type="hidden" name="fa_x_service_small_pic" id= "fa_x_service_small_pic" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_service_small_pic" id= "fa_x_service_small_pic" value="1">
<?php } ?>
...
<?php if (@$_POST["fa_x_service_large_pic"] == "0") { ?>
<input type="hidden" name="fa_x_service_large_pic" id= "fa_x_service_large_pic" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_service_large_pic" id= "fa_x_service_large_pic" value="1">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fservicesedit", "x_service_content", 35, 4, <?php echo ($services->service_content->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
