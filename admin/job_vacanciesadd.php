...
$job_vacancies_add = NULL; // Initialize page object first

class cjob_vacancies_add extends cjob_vacancies {
	...
	// Page object name
	var $PageObjName = 'job_vacancies_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["job_vacancy_id"] != "") {
				...
				$this->setKey("job_vacancy_id", $this->job_vacancy_id->CurrentValue); // Set up key
			} else {
				$this->setKey("job_vacancy_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "job_vacanciesview.php")
	...
	function LoadDefaultValues() {
		$this->job_vacancy_title->CurrentValue = NULL;
		$this->job_vacancy_title->OldValue = $this->job_vacancy_title->CurrentValue;
		$this->job_vacancy_description->CurrentValue = NULL;
		$this->job_vacancy_description->OldValue = $this->job_vacancy_description->CurrentValue;
		$this->job_vacancy_expiry_date->CurrentValue = NULL;
		$this->job_vacancy_expiry_date->OldValue = $this->job_vacancy_expiry_date->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// job_vacancy_title
		$this->job_vacancy_title->SetDbValueDef($rsnew, $this->job_vacancy_title->CurrentValue, "", FALSE);

		// job_vacancy_description
		$this->job_vacancy_description->SetDbValueDef($rsnew, $this->job_vacancy_description->CurrentValue, "", FALSE);

		// job_vacancy_expiry_date
		$this->job_vacancy_expiry_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->job_vacancy_expiry_date->CurrentValue, 5), ew_CurrentDate(), FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->job_vacancy_id->setDbValue($conn->Insert_ID());
				$rsnew['job_vacancy_id'] = $this->job_vacancy_id->DbValue;
			}
		} else {
...
// Create page object
if (!isset($job_vacancies_add)) $job_vacancies_add = new cjob_vacancies_add();

// Page init
$job_vacancies_add->Page_Init();

// Page main
$job_vacancies_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$job_vacancies_add->Page_Render();
?>
...
var CurrentForm = fjob_vacanciesadd = new ew_Form("fjob_vacanciesadd", "add");

// Validate form
fjob_vacanciesadd.Validate = function() {
...
fjob_vacanciesadd.Form_CustomValidate = 
...
fjob_vacanciesadd.ValidateRequired = true;
<?php } else { ?>
fjob_vacanciesadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fjob_vacanciesadd.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fjob_vacanciesadd.Lists["x_status"].Options = <?php echo json_encode($job_vacancies->status->Options()) ?>;
...
<?php $job_vacancies_add->ShowPageHeader(); ?>
<?php
$job_vacancies_add->ShowMessage();
?>
<form name="fjob_vacanciesadd" id="fjob_vacanciesadd" class="<?php echo $job_vacancies_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($job_vacancies_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $job_vacancies_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fjob_vacanciesadd", "x_job_vacancy_description", 35, 4, <?php echo ($job_vacancies->job_vacancy_description->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fjob_vacanciesadd", "x_job_vacancy_expiry_date", "%Y/%m/%d");
</script>
<?php } ?>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $job_vacancies_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fjob_vacanciesadd.Init();
</script>
<?php
$job_vacancies_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$job_vacancies_add->Page_Terminate();
?>
