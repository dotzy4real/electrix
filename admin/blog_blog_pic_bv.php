...
$blog_blog_pic_blobview = NULL; // Initialize page object first

class cblog_blog_pic_blobview extends cblog {
	...
	// Page object name
	var $PageObjName = 'blog_blog_pic_blobview';
	...
	function Page_Main() {
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('blog_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('blog_pic')));
				if (trim(strval($this->Recordset->fields('blog_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('blog_pic') . "\"");
				}
...
// Create page object
if (!isset($blog_blog_pic_blobview)) $blog_blog_pic_blobview = new cblog_blog_pic_blobview();

// Page init
$blog_blog_pic_blobview->Page_Init();

// Page main
$blog_blog_pic_blobview->Page_Main();
?>
<?php
$blog_blog_pic_blobview->Page_Terminate();
?>
