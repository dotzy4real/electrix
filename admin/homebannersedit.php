...
$homebanners_edit = NULL; // Initialize page object first

class chomebanners_edit extends chomebanners {
	...
	// Page object name
	var $PageObjName = 'homebanners_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->homebanner_id->CurrentValue == "")
			$this->Page_Terminate("homebannerslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->homebanner_id->FldIsDetailKey)
			$this->homebanner_id->setFormValue($objForm->GetValue("x_homebanner_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->homebanner_id->CurrentValue = $this->homebanner_id->FormValue;
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->homebanner_pic);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// homebanner_subtitle
			$this->homebanner_subtitle->SetDbValueDef($rsnew, $this->homebanner_subtitle->CurrentValue, "", $this->homebanner_subtitle->ReadOnly);

			// homebanner_maintitle
			$this->homebanner_maintitle->SetDbValueDef($rsnew, $this->homebanner_maintitle->CurrentValue, "", $this->homebanner_maintitle->ReadOnly);

			// homebanner_pic
			if (!($this->homebanner_pic->ReadOnly) && !$this->homebanner_pic->Upload->KeepFile) {
				$this->homebanner_pic->Upload->DbValue = $rsold['homebanner_pic']; // Get original value
			...
			// homebanner_button_text
			$this->homebanner_button_text->SetDbValueDef($rsnew, $this->homebanner_button_text->CurrentValue, "", $this->homebanner_button_text->ReadOnly);

			// homebanner_button_link
			$this->homebanner_button_link->SetDbValueDef($rsnew, $this->homebanner_button_link->CurrentValue, "", $this->homebanner_button_link->ReadOnly);
...
// Create page object
if (!isset($homebanners_edit)) $homebanners_edit = new chomebanners_edit();

// Page init
$homebanners_edit->Page_Init();

// Page main
$homebanners_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$homebanners_edit->Page_Render();
?>
...
var CurrentForm = fhomebannersedit = new ew_Form("fhomebannersedit", "edit");

// Validate form
fhomebannersedit.Validate = function() {
...
fhomebannersedit.Form_CustomValidate = 
...
fhomebannersedit.ValidateRequired = true;
<?php } else { ?>
fhomebannersedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fhomebannersedit.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fhomebannersedit.Lists["x_status"].Options = <?php echo json_encode($homebanners->status->Options()) ?>;
...
<?php $homebanners_edit->ShowPageHeader(); ?>
<?php
$homebanners_edit->ShowMessage();
?>
<form name="fhomebannersedit" id="fhomebannersedit" class="<?php echo $homebanners_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($homebanners_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $homebanners_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="homebanners">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<div>
<?php if ($homebanners->homebanner_id->Visible) { // homebanner_id ?>
	<div id="r_homebanner_id" class="form-group">
		<label id="elh_homebanners_homebanner_id" class="col-sm-2 control-label ewLabel"><?php echo $homebanners->homebanner_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $homebanners->homebanner_id->CellAttributes() ?>>
<span id="el_homebanners_homebanner_id">
<span<?php echo $homebanners->homebanner_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $homebanners->homebanner_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="homebanners" data-field="x_homebanner_id" name="x_homebanner_id" id="x_homebanner_id" value="<?php echo ew_HtmlEncode($homebanners->homebanner_id->CurrentValue) ?>">
<?php echo $homebanners->homebanner_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if (@$_POST["fa_x_homebanner_pic"] == "0") { ?>
<input type="hidden" name="fa_x_homebanner_pic" id= "fa_x_homebanner_pic" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_homebanner_pic" id= "fa_x_homebanner_pic" value="1">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $homebanners_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fhomebannersedit.Init();
</script>
<?php
$homebanners_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$homebanners_edit->Page_Terminate();
?>
