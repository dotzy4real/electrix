...
$feature_lists_add = NULL; // Initialize page object first

class cfeature_lists_add extends cfeature_lists {
	...
	// Page object name
	var $PageObjName = 'feature_lists_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["feature_list_id"] != "") {
				...
				$this->setKey("feature_list_id", $this->feature_list_id->CurrentValue); // Set up key
			} else {
				$this->setKey("feature_list_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "feature_listsview.php")
	...
	function LoadDefaultValues() {
		$this->feature_list_title->CurrentValue = NULL;
		$this->feature_list_title->OldValue = $this->feature_list_title->CurrentValue;
		$this->feature_list_snippet->CurrentValue = NULL;
		$this->feature_list_snippet->OldValue = $this->feature_list_snippet->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// feature_list_title
		$this->feature_list_title->SetDbValueDef($rsnew, $this->feature_list_title->CurrentValue, "", FALSE);

		// feature_list_snippet
		$this->feature_list_snippet->SetDbValueDef($rsnew, $this->feature_list_snippet->CurrentValue, "", FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->feature_list_id->setDbValue($conn->Insert_ID());
				$rsnew['feature_list_id'] = $this->feature_list_id->DbValue;
			}
		} else {
...
// Create page object
if (!isset($feature_lists_add)) $feature_lists_add = new cfeature_lists_add();

// Page init
$feature_lists_add->Page_Init();

// Page main
$feature_lists_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$feature_lists_add->Page_Render();
?>
...
var CurrentForm = ffeature_listsadd = new ew_Form("ffeature_listsadd", "add");

// Validate form
ffeature_listsadd.Validate = function() {
...
ffeature_listsadd.Form_CustomValidate = 
...
ffeature_listsadd.ValidateRequired = true;
<?php } else { ?>
ffeature_listsadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ffeature_listsadd.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ffeature_listsadd.Lists["x_status"].Options = <?php echo json_encode($feature_lists->status->Options()) ?>;
...
<?php $feature_lists_add->ShowPageHeader(); ?>
<?php
$feature_lists_add->ShowMessage();
?>
<form name="ffeature_listsadd" id="ffeature_listsadd" class="<?php echo $feature_lists_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($feature_lists_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $feature_lists_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $feature_lists_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
ffeature_listsadd.Init();
</script>
<?php
$feature_lists_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$feature_lists_add->Page_Terminate();
?>
