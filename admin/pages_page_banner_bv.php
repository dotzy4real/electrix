...
$pages_page_banner_blobview = NULL; // Initialize page object first

class cpages_page_banner_blobview extends cpages {
	...
	// Page object name
	var $PageObjName = 'pages_page_banner_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('pages', 'x_page_banner');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('page_banner');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('page_banner')));
				if (trim(strval($this->Recordset->fields('page_banner'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('page_banner') . "\"");
				}
...
// Create page object
if (!isset($pages_page_banner_blobview)) $pages_page_banner_blobview = new cpages_page_banner_blobview();

// Page init
$pages_page_banner_blobview->Page_Init();

// Page main
$pages_page_banner_blobview->Page_Main();
?>
<?php
$pages_page_banner_blobview->Page_Terminate();
?>
