...
$homebanners_homebanner_pic_blobview = NULL; // Initialize page object first

class chomebanners_homebanner_pic_blobview extends chomebanners {
	...
	// Page object name
	var $PageObjName = 'homebanners_homebanner_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('homebanners', 'x_homebanner_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('homebanner_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('homebanner_pic')));
				if (trim(strval($this->Recordset->fields('homebanner_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('homebanner_pic') . "\"");
				}
...
// Create page object
if (!isset($homebanners_homebanner_pic_blobview)) $homebanners_homebanner_pic_blobview = new chomebanners_homebanner_pic_blobview();

// Page init
$homebanners_homebanner_pic_blobview->Page_Init();

// Page main
$homebanners_homebanner_pic_blobview->Page_Main();
?>
<?php
$homebanners_homebanner_pic_blobview->Page_Terminate();
?>
