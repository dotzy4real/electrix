...
$home_about_home_about_pic_blobview = NULL; // Initialize page object first

class chome_about_home_about_pic_blobview extends chome_about {
	...
	// Page object name
	var $PageObjName = 'home_about_home_about_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('home_about', 'x_home_about_pic');
		...
		// SQL constructor in home_about class, home_aboutinfo.php
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('home_about_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('home_about_pic')));
				if (trim(strval($this->Recordset->fields('home_about_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('home_about_pic') . "\"");
				}
...
// Create page object
if (!isset($home_about_home_about_pic_blobview)) $home_about_home_about_pic_blobview = new chome_about_home_about_pic_blobview();

// Page init
$home_about_home_about_pic_blobview->Page_Init();

// Page main
$home_about_home_about_pic_blobview->Page_Main();
?>
<?php
$home_about_home_about_pic_blobview->Page_Terminate();
?>
