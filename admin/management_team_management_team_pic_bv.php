...
$management_team_management_team_pic_blobview = NULL; // Initialize page object first

class cmanagement_team_management_team_pic_blobview extends cmanagement_team {
	...
	// Page object name
	var $PageObjName = 'management_team_management_team_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('management_team', 'x_management_team_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('management_team_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('management_team_pic')));
				if (trim(strval($this->Recordset->fields('management_team_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('management_team_pic') . "\"");
				}
...
// Create page object
if (!isset($management_team_management_team_pic_blobview)) $management_team_management_team_pic_blobview = new cmanagement_team_management_team_pic_blobview();

// Page init
$management_team_management_team_pic_blobview->Page_Init();

// Page main
$management_team_management_team_pic_blobview->Page_Main();
?>
<?php
$management_team_management_team_pic_blobview->Page_Terminate();
?>
