...
$services_service_small_pic_blobview = NULL; // Initialize page object first

class cservices_service_small_pic_blobview extends cservices {
	...
	// Page object name
	var $PageObjName = 'services_service_small_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('services', 'x_service_small_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('service_small_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('service_small_pic')));
				if (trim(strval($this->Recordset->fields('service_small_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('service_small_pic') . "\"");
				}
...
// Create page object
if (!isset($services_service_small_pic_blobview)) $services_service_small_pic_blobview = new cservices_service_small_pic_blobview();

// Page init
$services_service_small_pic_blobview->Page_Init();

// Page main
$services_service_small_pic_blobview->Page_Main();
?>
<?php
$services_service_small_pic_blobview->Page_Terminate();
?>
