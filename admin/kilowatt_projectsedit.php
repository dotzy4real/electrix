...
$kilowatt_projects_edit = NULL; // Initialize page object first

class ckilowatt_projects_edit extends ckilowatt_projects {
	...
	// Page object name
	var $PageObjName = 'kilowatt_projects_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->kilowatt_project_id->CurrentValue == "")
			$this->Page_Terminate("kilowatt_projectslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->kilowatt_project_id->FldIsDetailKey)
			$this->kilowatt_project_id->setFormValue($objForm->GetValue("x_kilowatt_project_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->kilowatt_project_id->CurrentValue = $this->kilowatt_project_id->FormValue;
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->kilowatt_project_pic);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// kilowatt_project_pic
			if (!($this->kilowatt_project_pic->ReadOnly) && !$this->kilowatt_project_pic->Upload->KeepFile) {
				$this->kilowatt_project_pic->Upload->DbValue = $rsold['kilowatt_project_pic']; // Get original value
			...
			// kilowatt_project_title
			$this->kilowatt_project_title->SetDbValueDef($rsnew, $this->kilowatt_project_title->CurrentValue, "", $this->kilowatt_project_title->ReadOnly);
...
// Create page object
if (!isset($kilowatt_projects_edit)) $kilowatt_projects_edit = new ckilowatt_projects_edit();

// Page init
$kilowatt_projects_edit->Page_Init();

// Page main
$kilowatt_projects_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kilowatt_projects_edit->Page_Render();
?>
...
var CurrentForm = fkilowatt_projectsedit = new ew_Form("fkilowatt_projectsedit", "edit");

// Validate form
fkilowatt_projectsedit.Validate = function() {
...
fkilowatt_projectsedit.Form_CustomValidate = 
...
fkilowatt_projectsedit.ValidateRequired = true;
<?php } else { ?>
fkilowatt_projectsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkilowatt_projectsedit.Lists["x_kilowatt_service_category_id"] = {"LinkField":"x_kilowatt_service_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_kilowatt_service_category_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_projectsedit.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkilowatt_projectsedit.Lists["x_status"].Options = <?php echo json_encode($kilowatt_projects->status->Options()) ?>;
...
<?php $kilowatt_projects_edit->ShowPageHeader(); ?>
<?php
$kilowatt_projects_edit->ShowMessage();
?>
<form name="fkilowatt_projectsedit" id="fkilowatt_projectsedit" class="<?php echo $kilowatt_projects_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kilowatt_projects_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kilowatt_projects_edit->Token ?>">
<?php } ?>
...
<?php if ($kilowatt_projects->kilowatt_project_id->Visible) { // kilowatt_project_id ?>
	<div id="r_kilowatt_project_id" class="form-group">
		<label id="elh_kilowatt_projects_kilowatt_project_id" class="col-sm-2 control-label ewLabel"><?php echo $kilowatt_projects->kilowatt_project_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $kilowatt_projects->kilowatt_project_id->CellAttributes() ?>>
<span id="el_kilowatt_projects_kilowatt_project_id">
<span<?php echo $kilowatt_projects->kilowatt_project_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $kilowatt_projects->kilowatt_project_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="kilowatt_projects" data-field="x_kilowatt_project_id" name="x_kilowatt_project_id" id="x_kilowatt_project_id" value="<?php echo ew_HtmlEncode($kilowatt_projects->kilowatt_project_id->CurrentValue) ?>">
<?php echo $kilowatt_projects->kilowatt_project_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if (@$_POST["fa_x_kilowatt_project_pic"] == "0") { ?>
<input type="hidden" name="fa_x_kilowatt_project_pic" id= "fa_x_kilowatt_project_pic" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_kilowatt_project_pic" id= "fa_x_kilowatt_project_pic" value="1">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kilowatt_projects_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fkilowatt_projectsedit.Init();
</script>
<?php
$kilowatt_projects_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kilowatt_projects_edit->Page_Terminate();
?>
