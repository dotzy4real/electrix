...
class cservices_list extends cservices {
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->service_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->service_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function GetFilterList() {
		...
		// Initialize
		$sFilterList = "";
		$sFilterList = ew_Concat($sFilterList, $this->service_id->AdvancedSearch->ToJSON(), ","); // Field service_id
		$sFilterList = ew_Concat($sFilterList, $this->service_small_pic->AdvancedSearch->ToJSON(), ","); // Field service_small_pic
		$sFilterList = ew_Concat($sFilterList, $this->service_large_pic->AdvancedSearch->ToJSON(), ","); // Field service_large_pic
		$sFilterList = ew_Concat($sFilterList, $this->service_title->AdvancedSearch->ToJSON(), ","); // Field service_title
		$sFilterList = ew_Concat($sFilterList, $this->service_content->AdvancedSearch->ToJSON(), ","); // Field service_content
		$sFilterList = ew_Concat($sFilterList, $this->service_icon->AdvancedSearch->ToJSON(), ","); // Field service_icon
	...
	function RestoreFilterList() {
		...
		// Field service_small_pic
		$this->service_small_pic->AdvancedSearch->SearchValue = @$filter["x_service_small_pic"];
		$this->service_small_pic->AdvancedSearch->SearchOperator = @$filter["z_service_small_pic"];
		$this->service_small_pic->AdvancedSearch->SearchCondition = @$filter["v_service_small_pic"];
		$this->service_small_pic->AdvancedSearch->SearchValue2 = @$filter["y_service_small_pic"];
		$this->service_small_pic->AdvancedSearch->SearchOperator2 = @$filter["w_service_small_pic"];
		$this->service_small_pic->AdvancedSearch->Save();

		// Field service_large_pic
		$this->service_large_pic->AdvancedSearch->SearchValue = @$filter["x_service_large_pic"];
		$this->service_large_pic->AdvancedSearch->SearchOperator = @$filter["z_service_large_pic"];
		$this->service_large_pic->AdvancedSearch->SearchCondition = @$filter["v_service_large_pic"];
		$this->service_large_pic->AdvancedSearch->SearchValue2 = @$filter["y_service_large_pic"];
		$this->service_large_pic->AdvancedSearch->SearchOperator2 = @$filter["w_service_large_pic"];
		$this->service_large_pic->AdvancedSearch->Save();

		// Field service_title
		$this->service_title->AdvancedSearch->SearchValue = @$filter["x_service_title"];
		$this->service_title->AdvancedSearch->SearchOperator = @$filter["z_service_title"];
		$this->service_title->AdvancedSearch->SearchCondition = @$filter["v_service_title"];
		$this->service_title->AdvancedSearch->SearchValue2 = @$filter["y_service_title"];
		$this->service_title->AdvancedSearch->SearchOperator2 = @$filter["w_service_title"];
		$this->service_title->AdvancedSearch->Save();

		// Field service_content
		$this->service_content->AdvancedSearch->SearchValue = @$filter["x_service_content"];
		$this->service_content->AdvancedSearch->SearchOperator = @$filter["z_service_content"];
		$this->service_content->AdvancedSearch->SearchCondition = @$filter["v_service_content"];
		$this->service_content->AdvancedSearch->SearchValue2 = @$filter["y_service_content"];
		$this->service_content->AdvancedSearch->SearchOperator2 = @$filter["w_service_content"];
		$this->service_content->AdvancedSearch->Save();

		// Field service_icon
		$this->service_icon->AdvancedSearch->SearchValue = @$filter["x_service_icon"];
		$this->service_icon->AdvancedSearch->SearchOperator = @$filter["z_service_icon"];
		$this->service_icon->AdvancedSearch->SearchCondition = @$filter["v_service_icon"];
		$this->service_icon->AdvancedSearch->SearchValue2 = @$filter["y_service_icon"];
		$this->service_icon->AdvancedSearch->SearchOperator2 = @$filter["w_service_icon"];
		$this->service_icon->AdvancedSearch->Save();
	...
	function AdvancedSearchWhere($Default = FALSE) {
		global $Security;
		$sWhere = "";
		$this->BuildSearchSql($sWhere, $this->service_id, $Default, FALSE); // service_id
		$this->BuildSearchSql($sWhere, $this->service_small_pic, $Default, FALSE); // service_small_pic
		$this->BuildSearchSql($sWhere, $this->service_large_pic, $Default, FALSE); // service_large_pic
		$this->BuildSearchSql($sWhere, $this->service_title, $Default, FALSE); // service_title
		$this->BuildSearchSql($sWhere, $this->service_content, $Default, FALSE); // service_content
		$this->BuildSearchSql($sWhere, $this->service_icon, $Default, FALSE); // service_icon
		...
		if (!$Default && $this->Command == "search") {
			$this->service_id->AdvancedSearch->Save(); // service_id
			$this->service_small_pic->AdvancedSearch->Save(); // service_small_pic
			$this->service_large_pic->AdvancedSearch->Save(); // service_large_pic
			$this->service_title->AdvancedSearch->Save(); // service_title
			$this->service_content->AdvancedSearch->Save(); // service_content
			$this->service_icon->AdvancedSearch->Save(); // service_icon
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->service_small_pic, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->service_large_pic, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->service_title, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->service_content, $arKeywords, $type);
		return $sWhere;
	}
	...
	function CheckSearchParms() {
		...
		if ($this->BasicSearch->IssetSession())
			return TRUE;
		if ($this->service_id->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->service_small_pic->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->service_large_pic->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->service_title->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->service_content->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->service_icon->AdvancedSearch->IssetSession())
			return TRUE;
	...
	function ResetAdvancedSearchParms() {
		$this->service_id->AdvancedSearch->UnsetSession();
		$this->service_small_pic->AdvancedSearch->UnsetSession();
		$this->service_large_pic->AdvancedSearch->UnsetSession();
		$this->service_title->AdvancedSearch->UnsetSession();
		$this->service_content->AdvancedSearch->UnsetSession();
		$this->service_icon->AdvancedSearch->UnsetSession();
	...
	function RestoreSearchParms() {
		...
		$this->service_id->AdvancedSearch->Load();
		$this->service_small_pic->AdvancedSearch->Load();
		$this->service_large_pic->AdvancedSearch->Load();
		$this->service_title->AdvancedSearch->Load();
		$this->service_content->AdvancedSearch->Load();
		$this->service_icon->AdvancedSearch->Load();
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->service_small_pic); // service_small_pic
			$this->UpdateSort($this->service_large_pic); // service_large_pic
			$this->UpdateSort($this->service_title); // service_title
			$this->UpdateSort($this->service_icon); // service_icon
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->service_small_pic->setSort("");
				$this->service_large_pic->setSort("");
				$this->service_title->setSort("");
				$this->service_icon->setSort("");
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->service_id->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function LoadSearchValues() {
		...
		$this->service_id->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_service_id"]);
		if ($this->service_id->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->service_id->AdvancedSearch->SearchOperator = @$_GET["z_service_id"];

		// service_small_pic
		$this->service_small_pic->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_service_small_pic"]);
		if ($this->service_small_pic->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->service_small_pic->AdvancedSearch->SearchOperator = @$_GET["z_service_small_pic"];

		// service_large_pic
		$this->service_large_pic->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_service_large_pic"]);
		if ($this->service_large_pic->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->service_large_pic->AdvancedSearch->SearchOperator = @$_GET["z_service_large_pic"];

		// service_title
		$this->service_title->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_service_title"]);
		if ($this->service_title->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->service_title->AdvancedSearch->SearchOperator = @$_GET["z_service_title"];

		// service_content
		$this->service_content->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_service_content"]);
		if ($this->service_content->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->service_content->AdvancedSearch->SearchOperator = @$_GET["z_service_content"];

		// service_icon
		$this->service_icon->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_service_icon"]);
		if ($this->service_icon->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->service_icon->AdvancedSearch->SearchOperator = @$_GET["z_service_icon"];
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->service_small_pic->UseColorbox) {
				...
				$this->service_small_pic->LinkAttrs["data-rel"] = "services_x" . $this->RowCnt . "_service_small_pic";
			...
			if ($this->service_large_pic->UseColorbox) {
				...
				$this->service_large_pic->LinkAttrs["data-rel"] = "services_x" . $this->RowCnt . "_service_large_pic";
			...
			$this->service_id->EditValue = ew_HtmlEncode($this->service_id->AdvancedSearch->SearchValue);
			$this->service_id->PlaceHolder = ew_RemoveHtml($this->service_id->FldCaption());

			// service_small_pic
			$this->service_small_pic->EditAttrs["class"] = "form-control";
			$this->service_small_pic->EditCustomAttributes = "";
			$this->service_small_pic->EditValue = ew_HtmlEncode($this->service_small_pic->AdvancedSearch->SearchValue);
			$this->service_small_pic->PlaceHolder = ew_RemoveHtml($this->service_small_pic->FldCaption());

			// service_large_pic
			$this->service_large_pic->EditAttrs["class"] = "form-control";
			$this->service_large_pic->EditCustomAttributes = "";
			$this->service_large_pic->EditValue = ew_HtmlEncode($this->service_large_pic->AdvancedSearch->SearchValue);
			$this->service_large_pic->PlaceHolder = ew_RemoveHtml($this->service_large_pic->FldCaption());

			// service_title
			$this->service_title->EditAttrs["class"] = "form-control";
			$this->service_title->EditCustomAttributes = "";
			$this->service_title->EditValue = ew_HtmlEncode($this->service_title->AdvancedSearch->SearchValue);
...
fserviceslist.Lists["x_service_icon"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fserviceslist.Lists["x_service_icon"].Options = <?php echo json_encode($services->service_icon->Options()) ?>;
fserviceslist.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fserviceslist.Lists["x_status"].Options = <?php echo json_encode($services->status->Options()) ?>;
...
// Dynamic selection lists
fserviceslistsrch.Lists["x_service_icon"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fserviceslistsrch.Lists["x_service_icon"].Options = <?php echo json_encode($services->service_icon->Options()) ?>;
fserviceslistsrch.Lists["x_status"] = {"LinkField":"","Ajax":false,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fserviceslistsrch.Lists["x_status"].Options = <?php echo json_encode($services->status->Options()) ?>;
</script>
<script type="text/javascript">
...
<?php if ($services->service_icon->Visible) { // service_icon ?>
	<div id="xsc_service_icon" class="ewCell form-group">
		<label for="x_service_icon" class="ewSearchCaption ewLabel"><?php echo $services->service_icon->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_service_icon" id="z_service_icon" value="="></span>
...
if (is_array($services->service_icon->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = ew_SameStr($services->service_icon->AdvancedSearch->SearchValue, $arwrk[$rowcntwrk][0]) ? " selected" : "";
...
<?php if ($services->status->Visible) { // status ?>
	<div id="xsc_status" class="ewCell form-group">
		<label class="ewSearchCaption ewLabel"><?php echo $services->status->FldCaption() ?></label>
...
if (is_array($arwrk)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($services->status->AdvancedSearch->SearchValue) == strval($arwrk[$rowcntwrk][0])) ? " checked" : "";
		if ($selwrk <> "")
			$emptywrk = FALSE;
?>
...
<?php if ($services->service_id->Visible) { // service_id ?>
	<?php if ($services->SortUrl($services->service_id) == "") { ?>
		<th data-name="service_id"><div id="elh_services_service_id" class="services_service_id"><div class="ewTableHeaderCaption"><?php echo $services->service_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="service_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $services->SortUrl($services->service_id) ?>',1);"><div id="elh_services_service_id" class="services_service_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $services->service_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($services->service_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($services->service_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($services->service_small_pic->Visible) { // service_small_pic ?>
	<?php if ($services->SortUrl($services->service_small_pic) == "") { ?>
		<th data-name="service_small_pic"><div id="elh_services_service_small_pic" class="services_service_small_pic"><div class="ewTableHeaderCaption"><?php echo $services->service_small_pic->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="service_small_pic"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $services->SortUrl($services->service_small_pic) ?>',1);"><div id="elh_services_service_small_pic" class="services_service_small_pic">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $services->service_small_pic->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($services->service_small_pic->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($services->service_small_pic->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($services->service_large_pic->Visible) { // service_large_pic ?>
	<?php if ($services->SortUrl($services->service_large_pic) == "") { ?>
		<th data-name="service_large_pic"><div id="elh_services_service_large_pic" class="services_service_large_pic"><div class="ewTableHeaderCaption"><?php echo $services->service_large_pic->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="service_large_pic"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $services->SortUrl($services->service_large_pic) ?>',1);"><div id="elh_services_service_large_pic" class="services_service_large_pic">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $services->service_large_pic->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($services->service_large_pic->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($services->service_large_pic->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($services->service_title->Visible) { // service_title ?>
	<?php if ($services->SortUrl($services->service_title) == "") { ?>
		<th data-name="service_title"><div id="elh_services_service_title" class="services_service_title"><div class="ewTableHeaderCaption"><?php echo $services->service_title->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="service_title"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $services->SortUrl($services->service_title) ?>',1);"><div id="elh_services_service_title" class="services_service_title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $services->service_title->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($services->service_title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($services->service_title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($services->service_icon->Visible) { // service_icon ?>
	<?php if ($services->SortUrl($services->service_icon) == "") { ?>
		<th data-name="service_icon"><div id="elh_services_service_icon" class="services_service_icon"><div class="ewTableHeaderCaption"><?php echo $services->service_icon->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="service_icon"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $services->SortUrl($services->service_icon) ?>',1);"><div id="elh_services_service_icon" class="services_service_icon">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $services->service_icon->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($services->service_icon->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($services->service_icon->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($services->sort_order->Visible) { // sort_order ?>
	<?php if ($services->SortUrl($services->sort_order) == "") { ?>
		<th data-name="sort_order"><div id="elh_services_sort_order" class="services_sort_order"><div class="ewTableHeaderCaption"><?php echo $services->sort_order->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sort_order"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $services->SortUrl($services->sort_order) ?>',1);"><div id="elh_services_sort_order" class="services_sort_order">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $services->sort_order->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($services->sort_order->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($services->sort_order->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($services->status->Visible) { // status ?>
	<?php if ($services->SortUrl($services->status) == "") { ?>
		<th data-name="status"><div id="elh_services_status" class="services_status"><div class="ewTableHeaderCaption"><?php echo $services->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $services->SortUrl($services->status) ?>',1);"><div id="elh_services_status" class="services_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $services->status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($services->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($services->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
?>
	<?php if ($services->service_id->Visible) { // service_id ?>
		<td data-name="service_id"<?php echo $services->service_id->CellAttributes() ?>>
<span id="el<?php echo $services_list->RowCnt ?>_services_service_id" class="services_service_id">
...
<a id="<?php echo $services_list->PageObjName . "_row_" . $services_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($services->service_small_pic->Visible) { // service_small_pic ?>
		<td data-name="service_small_pic"<?php echo $services->service_small_pic->CellAttributes() ?>>
<span id="el<?php echo $services_list->RowCnt ?>_services_service_small_pic" class="services_service_small_pic">
<span>
<?php echo ew_GetFileViewTag($services->service_small_pic, $services->service_small_pic->ListViewValue()) ?>
	...
	<?php } ?>
	<?php if ($services->service_large_pic->Visible) { // service_large_pic ?>
		<td data-name="service_large_pic"<?php echo $services->service_large_pic->CellAttributes() ?>>
<span id="el<?php echo $services_list->RowCnt ?>_services_service_large_pic" class="services_service_large_pic">
<span>
<?php echo ew_GetFileViewTag($services->service_large_pic, $services->service_large_pic->ListViewValue()) ?>
	...
	<?php } ?>
	<?php if ($services->service_title->Visible) { // service_title ?>
		<td data-name="service_title"<?php echo $services->service_title->CellAttributes() ?>>
<span id="el<?php echo $services_list->RowCnt ?>_services_service_title" class="services_service_title">
...
<?php echo $services->service_title->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($services->service_icon->Visible) { // service_icon ?>
		<td data-name="service_icon"<?php echo $services->service_icon->CellAttributes() ?>>
<span id="el<?php echo $services_list->RowCnt ?>_services_service_icon" class="services_service_icon">
...
<?php echo $services->service_icon->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($services->sort_order->Visible) { // sort_order ?>
		<td data-name="sort_order"<?php echo $services->sort_order->CellAttributes() ?>>
<span id="el<?php echo $services_list->RowCnt ?>_services_sort_order" class="services_sort_order">
...
<?php echo $services->sort_order->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($services->status->Visible) { // status ?>
		<td data-name="status"<?php echo $services->status->CellAttributes() ?>>
<span id="el<?php echo $services_list->RowCnt ?>_services_status" class="services_status">
