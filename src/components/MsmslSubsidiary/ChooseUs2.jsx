...
function ChooseUs({ className }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await fetch(ApiUrl + "msmsl/getChooseUsItems");
            if (!response.ok) {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await fetch(ApiUrl + "msmsl/getChooseUsSection");
            if (!response.ok) {
              throw new Error(`HTTP error! status: ${response.status}`);
            }
            const result = await response.json();
      ...
      useEffect(() => {
          const fetchData = async () => {
            try {
              const response = await fetch(ApiUrl + "msmsl/getMissionEdge");
              if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
              }
              const result = await response.json();
            ...
            <section id="MsmslEdge" className={`why-choose-us-two msmsl ${className || ''}`}>
                ...
                <div className="outer-box">
                    ...
                    <div className="content-column col-lg-6 wow fadeInLeft" data-wow-delay="300ms">
                        ...
                        <div className="sec-title"> <span className="sub-title">{/*WHY CHOOSE US*/}{choose.msmsl_choose_us_icon_title}</span>
                            <h2 className="words-slide-up text-split">{/*We provide best efficient metering solution*/}{choose.msmsl_choose_us_title}</h2>
                        </div>
                        <div className="row">
                            ...
                            <div key={item.msmsl_choose_us_item_id} className="choose-block">
                                <div className="inner-box"> <i className={"icon " + item.msmsl_choose_us_item_icon}></i>
                                <div className="content">
                                    <h5 className="title">{item.msmsl_choose_us_item_title}</h5>
                                    <div className="text">{item.msmsl_choose_us_item_snippet}
                                    </div>
                                </div>
                                </div>
                            </div>
                        ...
                        <div className="why-us-tabs tabs-box">
                            ...
                            <div className="tabs-content">
                                ...
                                <div className="content-box">
                                    <div className="text">
                                    <div dangerouslySetInnerHTML={{ __html: mission.msmsl_mission }} />
                                        {/*
                                        <h4>Our Mission</h4>
...
{/*
                            ...
                            <div className={activeIndex === 2 ? "tab active-tab animated fadeIn" : "tab"} id="tab2">
                                <div className="inner-box">
                                <div className="content-box aboutContent">
                                    <div className="text">
                                        
                                    <div dangerouslySetInnerHTML={{ __html: mission.msmsl_edge }} />
                                        {/*Since inception in September, 2017 MSMSL has made giant strides in the provision of Metering Solutions catering to the domestic and international markets. We count the following as our major achievements, giving us the impetus to do even more in our product delivery.
                                        <br/><br/>
                                    <ul>
                                    <li><b>Product</b>
